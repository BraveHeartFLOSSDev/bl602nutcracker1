...
struct la_mem_format { // DWARF DIE: 1367
    uint32_t word[4];
};
...
struct co_list_hdr { // DWARF DIE: e5c
...
struct hostdesc { // DWARF DIE: 1d61
    uint32_t pbuf_addr;
...
struct tx_cfm_tag { // DWARF DIE: 18d0
    uint16_t pn[4];
    uint16_t sn;
    uint16_t timestamp;
    int8_t credits;
...
struct lmacdesc { // DWARF DIE: 1ecd
    struct tx_agg_desc * agg_desc;
    struct txl_buffer_tag * buffer;
    struct tx_hw_desc * hw_desc;
};

union anon_union.conflict1473_for_field_3 { // DWARF DIE: 1473
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

union anon_union.conflict1498_for_field_4 { // DWARF DIE: 1498
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};

union anon_union.conflict14bd_for_field_5 { // DWARF DIE: 14bd
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};

struct tx_hd { // DWARF DIE: 14e2
    uint32_t upatterntx;
...
struct tx_hw_desc { // DWARF DIE: 194f
    struct tx_cfm_tag * cfm_ptr;
    struct tx_hd thd;
};

struct umacdesc { // DWARF DIE: 1e3d
...
struct tx_compressed_policy_tbl { // DWARF DIE: 1448
    uint32_t upatterntx;
    uint32_t sec_user_control;
};

struct dma_desc { // DWARF DIE: 128d
    uint32_t src;
    uint32_t dest;
    uint16_t length;
    uint16_t ctrl;
...
struct tx_policy_tbl { // DWARF DIE: 13d7
    uint32_t upatterntx;
...
union anon_union.conflict29d5_for_field_0 { // DWARF DIE: 29d5
    struct tx_policy_tbl policy_tbl;
    struct tx_compressed_policy_tbl comp_pol_tbl;
};

struct txl_buffer_control { // DWARF DIE: 1e99
    union anon_union.conflict29d5_for_field_0 field_0;
    uint32_t mac_control_info;
    uint32_t phy_control_info;
};

struct tx_pbd { // DWARF DIE: 15ab
    uint32_t upatterntx;
    uint32_t next;
    uint32_t datastartptr;
    uint32_t dataendptr;
...
struct txl_buffer_tag { // DWARF DIE: 1f08
    uint32_t length;
    uint32_t lenheader;
    uint32_t lenpad;
    uint32_t flags;
...
struct tx_agg_desc { // DWARF DIE: 1980
    uint8_t reserved;
};

typedef struct dma_env_tag dma_env_tag, *Pdma_env_tag;

struct dma_env_tag { // DWARF DIE: 12e1
...
struct co_list { // DWARF DIE: e7d
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct mm_timer_env_tag { // DWARF DIE: 1239
    struct co_list prog;
};

typedef struct mm_timer_tag mm_timer_tag, *Pmm_timer_tag;

struct mm_timer_tag { // DWARF DIE: 11f8
    struct co_list_hdr list_hdr;
    void (* cb)(void *);
    void * env;
    uint32_t time;
...
struct rx_swdesc { // DWARF DIE: 170b
    struct co_list_hdr list_hdr;
...
struct phy_channel_info { // DWARF DIE: 133f
    uint32_t info1;
    uint32_t info2;
};

struct rx_pbd { // DWARF DIE: 17a0
    uint32_t upattern;
    uint32_t next;
    uint32_t datastartptr;
    uint32_t dataendptr;
...
struct rx_payloaddesc { // DWARF DIE: 1873
    struct rx_pbd pbd;
    uint32_t pd_status;
    uint32_t * buffer_rx;
    void * pbuf_holder[6];
...
struct rx_dmadesc { // DWARF DIE: 1803
    struct rx_hd hd;
...
struct ps_env_tag { // DWARF DIE: 3997
    _Bool ps_on;
    undefined field_0x1;
    ke_task_id_t taskid;
...
struct mm_chan_ctxt_add_req { // DWARF DIE: 1af3
    uint8_t band;
    uint8_t type;
...
struct debug_env_tag { // DWARF DIE: 139f
...
struct mblock_free { // DWARF DIE: 2112
};

struct ke_env_tag { // DWARF DIE: 20c3
...
struct co_read16_struct { // DWARF DIE: 3c4c
    uint16_t val;
};
...
struct txl_list { // DWARF DIE: 2833
...
struct txl_cntrl_env_tag { // DWARF DIE: 287b
...
struct txl_frame_cfm_tag { // DWARF DIE: 2acb
    void (* cfm_func)(void *, uint32_t);
    void * env;
};

struct txl_frame_desc_tag { // DWARF DIE: 2af3
    struct txdesc txdesc;
...
struct anon_struct.conflict34f2 { // DWARF DIE: 34f2
    uint32_t dummy;
...
struct anon_struct.conflict3425 { // DWARF DIE: 3425
...
union anon_union.conflict35ed { // DWARF DIE: 35ed
...
struct Cipher_t { // DWARF DIE: 306d
    UINT8 wep40:1;
    UINT8 wep104:1;
    UINT8 tkip:1;
    UINT8 ccmp:1;
...
struct chan_tbtt_tag { // DWARF DIE: 1b92
    struct co_list_hdr list_hdr;
    uint32_t time;
    uint8_t vif_index;
    uint8_t priority;
...
struct mobility_domain { // DWARF DIE: 321d
    uint16_t mdid;
...
struct SecurityMode_t { // DWARF DIE: 30d3
    UINT16 noRsn:1;
    UINT16 wepStatic:1;
    UINT16 wepDynamic:1;
    UINT16 wpa:1;
...
struct mac_bss_info { // DWARF DIE: 3245
...
union anon_union.conflict35ed_for_u { // DWARF DIE: 35ed
    struct anon_struct.conflict3425 sta;
    struct anon_struct.conflict34f2 ap;
};

struct vif_info_tag { // DWARF DIE: 3610
    struct co_list_hdr list_hdr;
    uint32_t prevent_sleep;
...
struct chan_ctxt_tag { // DWARF DIE: 1be1
    struct co_list_hdr list_hdr;
...
struct vif_mgmt_env_tag { // DWARF DIE: 374b
...
struct hal_dma_env_tag { // DWARF DIE: 2d5b
...
struct hal_dma_desc_tag { // DWARF DIE: 2d14
...
struct txl_frame_env_tag { // DWARF DIE: 2b47
...
struct bam_env_tag { // DWARF DIE: 10aa
    uint32_t pkt_cnt;
...
struct sta_mgmt_ba_info { // DWARF DIE: 25b4
...
struct rc_rate_stats { // DWARF DIE: 2edf
    uint16_t attempts;
    uint16_t success;
...
union anon_union.conflict2f7c_for_rate_map { // DWARF DIE: 2f7c
    uint8_t ht[4];
};

struct step { // DWARF DIE: 2f55
    uint32_t tp;
    uint16_t idx;
    undefined field_0x6;
    undefined field_0x7;
...
struct rc_sta_stats { // DWARF DIE: 244e
...
struct sta_pol_tbl_cntl { // DWARF DIE: 23ff
...
struct sta_mgmt_sec_info { // DWARF DIE: 23c4
...
struct sta_info_tag { // DWARF DIE: 2603
    struct co_list_hdr list_hdr;
    uint32_t bcn_int;
...
struct sta_info_env_tag { // DWARF DIE: 27ec
    struct co_list free_sta_list;
};

typedef struct chan_env_tag chan_env_tag, *Pchan_env_tag;

struct chan_env_tag { // DWARF DIE: 1c71
...
struct me_chan_config_req { // DWARF DIE: 2fc1
...
struct me_env_tag { // DWARF DIE: 3189
...
struct ke_msg_handler { // DWARF DIE: fac
    ke_msg_id_t id;
    undefined field_0x2;
    undefined field_0x3;
...
struct ke_state_handler { // DWARF DIE: fd8
...
struct mm_env_tag { // DWARF DIE: 37ca
...
struct scan_env_tag { // DWARF DIE: 2e01
...
struct scan_probe_req_ie_tag { // DWARF DIE: 2dbc
    struct dma_desc dma_desc;
    struct tx_pbd pbd;
    uint32_t buf[50];
};
...
struct txl_buffer_list_tag { // DWARF DIE: 2924
...
struct txl_buffer_hw_desc_tag { // DWARF DIE: 28fc
    struct dma_desc dma_desc;
    struct tx_pbd pbd;
};
...
union anon_union.conflict29d5 { // DWARF DIE: 29d5
    struct tx_policy_tbl policy_tbl;
    struct tx_compressed_policy_tbl comp_pol_tbl;
};
...
struct txl_buffer_idx_tag { // DWARF DIE: 294c
    uint32_t used_area;
    uint32_t free;
    uint32_t free_size;
    uint32_t last;
...
struct txl_buffer_env_tag { // DWARF DIE: 29f7
...
union anon_union.conflict14bd { // DWARF DIE: 14bd
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};
...
union anon_union.conflict1498 { // DWARF DIE: 1498
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};
...
union anon_union.conflict1473 { // DWARF DIE: 1473
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};
...
struct rxu_mgt_ind { // DWARF DIE: 38bf
    uint16_t length;
    uint16_t framectrl;
...
struct rx_cntrl_ipcdesc { // DWARF DIE: 2252
    uint32_t host_id;
};
...
struct rx_cntrl_pm_mon { // DWARF DIE: 229a
    struct mac_addr addr;
    uint8_t pm_state;
    _Bool mon;
};

struct rx_cntrl_dupli { // DWARF DIE: 226f
    struct mac_addr last_src_addr;
    uint16_t last_seq_cntl;
};

struct rx_cntrl_rx_status { // DWARF DIE: 2157
    uint16_t frame_cntl;
    uint16_t seq_cntl;
    uint16_t sn;
    uint8_t fn;
...
struct rxu_cntrl_env_tag { // DWARF DIE: 22d3
