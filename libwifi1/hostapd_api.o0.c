...
union anon_union.conflict104_for___value { // DWARF DIE: 104
    wint_t __wch;
    uchar __wchb[4];
};

struct _mbstate_t { // DWARF DIE: 136
    int __count;
...
union anon_union.conflict104 { // DWARF DIE: 104
    wint_t __wch;
    uchar __wchb[4];
};
...
struct la_mem_format { // DWARF DIE: eaf
    uint32_t word[4];
};
...
struct co_list_hdr { // DWARF DIE: e3e
    struct co_list_hdr * next;
};

struct mac_addr { // DWARF DIE: 996
    uint16_t array[3];
};

struct hostdesc { // DWARF DIE: 15b3
    uint32_t pbuf_addr;
...
struct tx_cfm_tag { // DWARF DIE: 1418
    uint16_t pn[4];
    uint16_t sn;
    uint16_t timestamp;
    int8_t credits;
...
union anon_union.conflict1005_for_field_5 { // DWARF DIE: 1005
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};

union anon_union.conflictfe0_for_field_4 { // DWARF DIE: fe0
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};

union anon_union.conflictfbb_for_field_3 { // DWARF DIE: fbb
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

struct tx_hd { // DWARF DIE: 102a
    uint32_t upatterntx;
...
struct tx_hw_desc { // DWARF DIE: 1497
    struct tx_cfm_tag * cfm_ptr;
    struct tx_hd thd;
};

struct lmacdesc { // DWARF DIE: 171f
    struct tx_agg_desc * agg_desc;
    struct txl_buffer_tag * buffer;
    struct tx_hw_desc * hw_desc;
};

struct umacdesc { // DWARF DIE: 168f
...
struct tx_compressed_policy_tbl { // DWARF DIE: f90
    uint32_t upatterntx;
    uint32_t sec_user_control;
};

struct dma_desc { // DWARF DIE: dad
    uint32_t src;
    uint32_t dest;
    uint16_t length;
    uint16_t ctrl;
...
struct tx_policy_tbl { // DWARF DIE: f1f
    uint32_t upatterntx;
...
union anon_union.conflict2783_for_field_0 { // DWARF DIE: 2783
    struct tx_policy_tbl policy_tbl;
    struct tx_compressed_policy_tbl comp_pol_tbl;
};

struct txl_buffer_control { // DWARF DIE: 16eb
    union anon_union.conflict2783_for_field_0 field_0;
    uint32_t mac_control_info;
    uint32_t phy_control_info;
};

struct txl_buffer_tag { // DWARF DIE: 175a
    uint32_t length;
    uint32_t lenheader;
    uint32_t lenpad;
    uint32_t flags;
...
struct tx_agg_desc { // DWARF DIE: 14c8
    uint8_t reserved;
};

typedef struct dma_env_tag dma_env_tag, *Pdma_env_tag;

struct dma_env_tag { // DWARF DIE: e01
...
struct co_list { // DWARF DIE: e5f
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct mm_timer_env_tag { // DWARF DIE: 1b9f
    struct co_list prog;
};

typedef struct mm_timer_tag mm_timer_tag, *Pmm_timer_tag;

struct mm_timer_tag { // DWARF DIE: 1b5e
    struct co_list_hdr list_hdr;
    void (* cb)(void *);
    void * env;
    uint32_t time;
...
struct rx_swdesc { // DWARF DIE: 1253
    struct co_list_hdr list_hdr;
...
struct phy_channel_info { // DWARF DIE: e87
    uint32_t info1;
    uint32_t info2;
};

struct rx_pbd { // DWARF DIE: 12e8
    uint32_t upattern;
    uint32_t next;
    uint32_t datastartptr;
    uint32_t dataendptr;
...
struct rx_payloaddesc { // DWARF DIE: 13bb
    struct rx_pbd pbd;
    uint32_t pd_status;
    uint32_t * buffer_rx;
    void * pbuf_holder[6];
...
struct rx_dmadesc { // DWARF DIE: 134b
    struct rx_hd hd;
...
struct ps_env_tag { // DWARF DIE: 3536
    _Bool ps_on;
    undefined field_0x1;
    ke_task_id_t taskid;
...
struct mm_chan_ctxt_add_req { // DWARF DIE: 1bc6
    uint8_t band;
    uint8_t type;
...
struct debug_env_tag { // DWARF DIE: ee7
...
struct mblock_free { // DWARF DIE: 1ec0
};

struct ke_env_tag { // DWARF DIE: 1e71
...
struct txl_list { // DWARF DIE: 25e1
...
struct txl_cntrl_env_tag { // DWARF DIE: 2629
...
struct anon_struct.conflict3191 { // DWARF DIE: 3191
...
struct anon_struct.conflicta63 { // DWARF DIE: a63
    uint32_t tx_key[2];
    uint32_t rx_key[2];
};

union anon_union.conflictac3_for_u { // DWARF DIE: ac3
    struct anon_struct.conflicta63 mic;
    struct anon_struct.conflicta9a mfp;
};

struct Cipher_t { // DWARF DIE: 2dd9
    UINT8 wep40:1;
    UINT8 wep104:1;
    UINT8 tkip:1;
    UINT8 ccmp:1;
...
struct chan_tbtt_tag { // DWARF DIE: 1c75
    struct co_list_hdr list_hdr;
    uint32_t time;
    uint8_t vif_index;
    uint8_t priority;
...
struct scan_chan_tag { // DWARF DIE: c6f
    uint16_t freq;
    uint8_t band;
    uint8_t flags;
    int8_t tx_power;
...
struct mac_edca_param_set { // DWARF DIE: c36
    uint8_t qos_info;
    uint8_t acm;
    undefined field_0x2;
    undefined field_0x3;
...
struct mac_rateset { // DWARF DIE: a1b
    uint8_t length;
    uint8_t array[12];
};

struct txl_frame_cfm_tag { // DWARF DIE: 2879
    void (* cfm_func)(void *, uint32_t);
    void * env;
};

struct txl_frame_desc_tag { // DWARF DIE: 28a1
    struct txdesc txdesc;
...
struct key_info_tag { // DWARF DIE: ae8
    uint64_t rx_pn[9];
    uint64_t tx_pn;
...
struct anon_struct.conflict325e { // DWARF DIE: 325e
    uint32_t dummy;
...
union anon_union.conflict3359_for_u { // DWARF DIE: 3359
    struct anon_struct.conflict3191 sta;
    struct anon_struct.conflict325e ap;
};

struct mac_ssid { // DWARF DIE: 9c6
    uint8_t length;
    uint8_t array[32];
    uint8_t array_tail[1];
};

struct mac_htcapability { // DWARF DIE: b6e
...
struct SecurityMode_t { // DWARF DIE: 2e3f
    UINT16 noRsn:1;
    UINT16 wepStatic:1;
    UINT16 wepDynamic:1;
    UINT16 wpa:1;
...
struct mobility_domain { // DWARF DIE: 2f89
    uint16_t mdid;
    uint8_t ft_capability_policy;
    undefined field_0x3;
};

struct mac_bss_info { // DWARF DIE: 2fb1
...
struct vif_info_tag { // DWARF DIE: 337c
    struct co_list_hdr list_hdr;
    uint32_t prevent_sleep;
...
struct chan_ctxt_tag { // DWARF DIE: 1cc4
    struct co_list_hdr list_hdr;
...
struct vif_mgmt_env_tag { // DWARF DIE: 34b7
...
union anon_union.conflict3359 { // DWARF DIE: 3359
    struct anon_struct.conflict3191 sta;
    struct anon_struct.conflict325e ap;
};
...
struct hal_dma_env_tag { // DWARF DIE: 2ae8
...
struct hal_dma_desc_tag { // DWARF DIE: 2aa1
...
struct txl_frame_env_tag { // DWARF DIE: 28f5
...
struct bam_env_tag { // DWARF DIE: 1a53
    uint32_t pkt_cnt;
...
struct sta_mgmt_ba_info { // DWARF DIE: 2362
...
union anon_union.conflict2ce8_for_rate_map { // DWARF DIE: 2ce8
    uint8_t ht[4];
};

struct step { // DWARF DIE: 2cc1
    uint32_t tp;
    uint16_t idx;
    undefined field_0x6;
    undefined field_0x7;
...
struct rc_rate_stats { // DWARF DIE: 2c4b
    uint16_t attempts;
    uint16_t success;
...
struct rc_sta_stats { // DWARF DIE: 21fc
...
struct sta_pol_tbl_cntl { // DWARF DIE: 21ad
...
struct mac_vhtcapability { // DWARF DIE: be1
...
struct mac_sta_info { // DWARF DIE: cb7
    struct mac_rateset rate_set;
    undefined field_0xd;
    undefined field_0xe;
    undefined field_0xf;
...
struct sta_info_env_tag { // DWARF DIE: 259a
    struct co_list free_sta_list;
};

typedef struct chan_env_tag chan_env_tag, *Pchan_env_tag;

struct chan_env_tag { // DWARF DIE: 1d54
...
struct me_chan_config_req { // DWARF DIE: 2d2d
...
struct me_env_tag { // DWARF DIE: 2ef5
...
union anon_union.conflictac3 { // DWARF DIE: ac3
    struct anon_struct.conflicta63 mic;
    struct anon_struct.conflicta9a mfp;
};
...
struct ke_msg_handler { // DWARF DIE: 1955
    ke_msg_id_t id;
    undefined field_0x2;
    undefined field_0x3;
...
struct ke_state_handler { // DWARF DIE: 1981
...
struct scan_env_tag { // DWARF DIE: 2b8e
...
struct scan_probe_req_ie_tag { // DWARF DIE: 2b49
    struct dma_desc dma_desc;
    struct tx_pbd pbd;
    uint32_t buf[50];
};
...
struct txl_buffer_list_tag { // DWARF DIE: 26d2
...
struct txl_buffer_hw_desc_tag { // DWARF DIE: 26aa
    struct dma_desc dma_desc;
    struct tx_pbd pbd;
};
...
struct txl_buffer_idx_tag { // DWARF DIE: 26fa
    uint32_t used_area;
    uint32_t free;
    uint32_t free_size;
    uint32_t last;
...
union anon_union.conflict2783 { // DWARF DIE: 2783
    struct tx_policy_tbl policy_tbl;
    struct tx_compressed_policy_tbl comp_pol_tbl;
};
...
struct txl_buffer_env_tag { // DWARF DIE: 27a5
...
union anon_union.conflictfe0 { // DWARF DIE: fe0
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};
...
union anon_union.conflict1005 { // DWARF DIE: 1005
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};
...
union anon_union.conflictfbb { // DWARF DIE: fbb
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};
...
struct _rand48 { // DWARF DIE: 709
    ushort _seed[3];
    ushort _mult[3];
    ushort _add;
};

struct anon_struct.conflict752 { // DWARF DIE: 752
    uint _unused_rand;
    char * _strtok_last;
...
union anon_union.conflict8de_for__new { // DWARF DIE: 8de
    struct anon_struct.conflict752 _reent;
    struct anon_struct.conflict752 _unused;
};

struct _Bigint { // DWARF DIE: 18c
...
struct _glue { // DWARF DIE: 6c4
    struct _glue * _next;
    int _niobs;
    __FILE * _iobs;
};

struct __sbuf { // DWARF DIE: 334
    uchar * _base;
    int _size;
};

struct __sFILE { // DWARF DIE: 362
    uchar * _p;
    int _r;
    int _w;
    short _flags;
...
struct _on_exit_args { // DWARF DIE: 27f
    void * _fnargs[32];
    void * _dso_handle[32];
    __ULong _fntypes;
    __ULong _is_cxa;
...
struct _reent { // DWARF DIE: 4ce
    ...
    int _cvtlen;
    char * _cvtbuf;
    union anon_union.conflict8de_for__new _new;
...
union anon_union.conflict8de { // DWARF DIE: 8de
    struct anon_struct.conflict752 _reent;
    struct anon_struct.conflict752 _unused;
...
struct rx_cntrl_ipcdesc { // DWARF DIE: 2000
    uint32_t host_id;
};
...
struct rx_cntrl_pm_mon { // DWARF DIE: 2048
    struct mac_addr addr;
    uint8_t pm_state;
    _Bool mon;
};

struct rx_cntrl_dupli { // DWARF DIE: 201d
    struct mac_addr last_src_addr;
    uint16_t last_seq_cntl;
};

struct rx_cntrl_rx_status { // DWARF DIE: 1f05
    uint16_t frame_cntl;
    uint16_t seq_cntl;
    uint16_t sn;
    uint8_t fn;
...
struct rxu_cntrl_env_tag { // DWARF DIE: 2081
