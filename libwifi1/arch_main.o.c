...
struct ipc_a2e_msg { // DWARF DIE: 1459
    uint32_t dummy_word;
    uint32_t msg[127];
};
...
struct hostdesc { // DWARF DIE: 11c4
    uint32_t pbuf_addr;
...
struct txdesc_host { // DWARF DIE: 13cd
    uint32_t ready;
...
struct ipc_shared_env_tag { // DWARF DIE: 1497
...
union anon_union.conflict129_for___value { // DWARF DIE: 129
    wint_t __wch;
    uchar __wchb[4];
};

struct _mbstate_t { // DWARF DIE: 15b
    int __count;
...
union anon_union.conflict129 { // DWARF DIE: 129
    wint_t __wch;
    uchar __wchb[4];
};
...
struct la_mem_format { // DWARF DIE: c4b
    uint32_t word[4];
};
...
struct co_list_hdr { // DWARF DIE: bda
    struct co_list_hdr * next;
};

union anon_union.conflictcbb_for_field_3 { // DWARF DIE: cbb
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

struct tx_cfm_tag { // DWARF DIE: 1039
    uint16_t pn[4];
    uint16_t sn;
    uint16_t timestamp;
    int8_t credits;
...
union anon_union.conflictd05_for_field_5 { // DWARF DIE: d05
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};

union anon_union.conflictce0_for_field_4 { // DWARF DIE: ce0
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};

struct tx_hd { // DWARF DIE: d2a
    uint32_t upatterntx;
...
struct tx_hw_desc { // DWARF DIE: 10a8
    struct tx_cfm_tag * cfm_ptr;
    struct tx_hd thd;
};

struct lmacdesc { // DWARF DIE: 1307
    struct tx_agg_desc * agg_desc;
    struct txl_buffer_tag * buffer;
    struct tx_hw_desc * hw_desc;
};

struct umacdesc { // DWARF DIE: 12a0
...
struct txl_buffer_tag { // DWARF DIE: 1342
};

struct txl_buffer_control { // DWARF DIE: 12fc
};

struct tx_agg_desc { // DWARF DIE: 10d9
    uint8_t reserved;
};

typedef struct dma_desc dma_desc, *Pdma_desc;

struct dma_desc { // DWARF DIE: b49
    uint32_t src;
    uint32_t dest;
    uint16_t length;
    uint16_t ctrl;
...
struct dma_env_tag { // DWARF DIE: b9d
...
struct debug_env_tag { // DWARF DIE: c83
...
struct mblock_free { // DWARF DIE: 1551
};

struct co_list { // DWARF DIE: bfb
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct ke_env_tag { // DWARF DIE: 1502
...
struct __locale_t { // DWARF DIE: 924
};

typedef struct rx_swdesc rx_swdesc, *Prx_swdesc;

struct rx_swdesc { // DWARF DIE: efe
};

typedef struct phy_channel_info phy_channel_info, *Pphy_channel_info;

struct phy_channel_info { // DWARF DIE: c23
    uint32_t info1;
    uint32_t info2;
};
...
struct bl_env_tag { // DWARF DIE: 987
...
union anon_union.conflictcbb { // DWARF DIE: cbb
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};
...
union anon_union.conflictce0 { // DWARF DIE: ce0
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};
...
struct rx_payloaddesc { // DWARF DIE: fdc
    struct rx_pbd pbd;
    uint32_t pd_status;
    uint32_t * buffer_rx;
    void * pbuf_holder[6];
...
struct rx_dmadesc { // DWARF DIE: f6c
    struct rx_hd hd;
...
union anon_union.conflictd05 { // DWARF DIE: d05
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};
...
struct ipc_emb_env_tag { // DWARF DIE: 161f
...
struct _rand48 { // DWARF DIE: 71a
    ushort _seed[3];
    ushort _mult[3];
    ushort _add;
};

struct _glue { // DWARF DIE: 6d5
    struct _glue * _next;
    int _niobs;
    __FILE * _iobs;
};

struct anon_struct.conflict8a8 { // DWARF DIE: 8a8
    uchar * _nextf[30];
    uint _nmalloc[30];
};
...
struct anon_struct.conflict763 { // DWARF DIE: 763
    uint _unused_rand;
    char * _strtok_last;
...
union anon_union.conflict8ef_for__new { // DWARF DIE: 8ef
...
union anon_union.conflict8ef { // DWARF DIE: 8ef
    struct anon_struct.conflict763 _reent;
    struct anon_struct.conflict8a8 _unused;
};
...
{
  if (enable == 0) {
    _DAT_44b00400 &= 0xffffffef;
  }
  else {
    _DAT_44b00400 |= 0xf0000010;
  }
  if ((int)(_DAT_44b00400 << 4) < 0) {
    _DAT_44b00400 &= 0xfbffffff;
  }
  else {
...

{
  ...
  if (iRam00000000 != enable) {
    if (iRam00000000 == 0) {
      ...
      if (iRam00000000 == 1) {
        _L0();
        iRam00000000 = extraout_a0_00;
        _L0();
        if (1000 < extraout_a0_01) {
          _L0();
          FUN_00010184();
          _L0();
        }
...

{
  ...
  _L0();
  iRam00000000 = (iRam00000000 - *unaff_s0) + extraout_a0;
  *unaff_s1 = unaff_s2;
  _L0();
...

{
  ...
  _L0();
  iRam00000000 = (iRam00000000 - *unaff_s0) + extraout_a0_00;
  _L0();
  if (1000 < extraout_a0_01) {
    _L0();
    FUN_00010184();
...

{
  ...
  while( true ) {
    if (_DAT_44b00120 << 0xc < 0) {
      ...
      _DAT_44900084 &= 0xfffffffe;
    }
    if (uRam00000000 == 0) {
      _L0();
    }
    if ((uRam00000000 & 0xf) == 0) {
      uVar1 = _DAT_40007018 >> 0x18 & 7;
      if (uVar1 != 0) {
        if (uVar1 != 3) {
          _DAT_40000014 |= 0x40000;
        }
        if ((uVar1 != 0) && (uVar1 != 3)) {
          _DAT_40002040 &= 0xfffffffc;
          _DAT_40002044 &= 0xfffffffe;
        }
      }
      if ((_DAT_40007018 >> 0x18 & 4) != 0) {
        _DAT_40000014 |= 0x5c2000;
  ...
  do {
    ...
    do {
      ...
      *unaff_s4 = uVar1 + 1;
      ...
      _L0(0);
      if (*(int *)(unaff_s2 + 0x120) << 0xc < 0) {
        uVar1 = *(uint *)(unaff_s0 + 0x84) | 1;
      }
      else {
        uVar1 = *(uint *)(unaff_s0 + 0x84) & 0xfffffffe;
      }
      *(uint *)(unaff_s0 + 0x84) = uVar1;
    } while (*unaff_s3 != 0);
  } while( true );
}
