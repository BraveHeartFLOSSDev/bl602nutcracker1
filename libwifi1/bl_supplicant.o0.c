...
typedef union anon_union.conflict28a2_for_key anon_union.conflict28a2_for_key, *Panon_union.conflict28a2_for_key;
...
union anon_union.conflict28a2_for_key { // DWARF DIE: 28a2
    IEEEtypes_MacAddr_t Bssid;
    char Ssid[32];
};

struct pmkElement_t { // DWARF DIE: 28d4
    union anon_union.conflict28a2_for_key key;
    UINT8 PMK[32];
    UINT8 length;
    UINT8 psk_length;
...
typedef union anon_union.conflict28a2 anon_union.conflict28a2, *Panon_union.conflict28a2;

union anon_union.conflict28a2 { // DWARF DIE: 28a2
    IEEEtypes_MacAddr_t Bssid;
    char Ssid[32];
};
...
struct la_mem_format { // DWARF DIE: a28
    uint32_t word[4];
};
...
struct dma_env_tag { // DWARF DIE: bf5
...
struct co_list_hdr { // DWARF DIE: c32
    struct co_list_hdr * next;
};

struct co_list { // DWARF DIE: c53
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct mm_timer_env_tag { // DWARF DIE: 2240
    struct co_list prog;
};

typedef struct mm_timer_tag mm_timer_tag, *Pmm_timer_tag;

struct mm_timer_tag { // DWARF DIE: 21ff
...
struct Cipher_t { // DWARF DIE: 1feb
    UINT8 wep40:1;
    UINT8 wep104:1;
    UINT8 tkip:1;
    UINT8 ccmp:1;
...
struct KeyData_t { // DWARF DIE: 1d1e
    UINT8 Key[16];
    UINT8 RxMICKey[8];
    UINT8 TxMICKey[8];
    UINT32 TxIV32;
...
struct CommonMlmeData_t { // DWARF DIE: 2bd4
...
struct Operation_t { // DWARF DIE: 2cbe
    UINT16 keyExchange:1;
    UINT16 authenticate:1;
    UINT16 reserved:14;
};

struct apRsnConfig_t { // DWARF DIE: 2d09
...
struct BssConfig_t { // DWARF DIE: 2e0f
...
struct BssData_t { // DWARF DIE: 2e5f
...
struct apInfo_t { // DWARF DIE: 2eff
...
struct _txQingInfo_t { // DWARF DIE: 2c94
...
struct SecurityMode_t { // DWARF DIE: 2051
    UINT16 noRsn:1;
    UINT16 wepStatic:1;
    UINT16 wepDynamic:1;
    UINT16 wpa:1;
...
struct apKeyMgmtInfoStaRom_t { // DWARF DIE: 2ae6
    UINT16 staRsnCap;
...
struct apKeyMgmtInfoSta_t { // DWARF DIE: 2b3d
...
struct staData_t { // DWARF DIE: 2ecb
...
struct debug_env_tag { // DWARF DIE: a88
...
typedef union anon_union.conflict1ec3_for_keyEncypt anon_union.conflict1ec3_for_keyEncypt, *Panon_union.conflict1ec3_for_keyEncypt;
...
struct key_Type_AES_CMAC_t { // DWARF DIE: 1e86
    UINT8 ipn[6];
    UINT8 reserved[2];
    UINT8 key[16];
};

struct key_Type_WAPI_t { // DWARF DIE: 1e2f
    UINT8 keyIndex;
    UINT8 isDefKey;
    UINT8 key[16];
    UINT8 mickey[16];
...
struct key_Type_WEP_t { // DWARF DIE: 1dbf
    UINT8 keyIndex;
    UINT8 isDefaultTx;
    UINT8 key[13];
};

struct key_Type_TKIP_t { // DWARF DIE: 1d82
    UINT8 key[16];
    UINT8 txMicKey[8];
    UINT8 rxMicKey[8];
};

struct key_Type_AES_t { // DWARF DIE: 1e0c
    UINT8 key[16];
};

union anon_union.conflict1ec3_for_keyEncypt { // DWARF DIE: 1ec3
...
struct key_MgtMaterial_t { // DWARF DIE: 1f09
    UINT16 keyType;
    UINT16 keyInfo;
    UINT16 keyLen;
    union anon_union.conflict1ec3_for_keyEncypt keyEncypt;
};

typedef union anon_union.conflict1ec3 anon_union.conflict1ec3, *Panon_union.conflict1ec3;

union anon_union.conflict1ec3 { // DWARF DIE: 1ec3
    struct key_Type_TKIP_t TKIP;
    struct key_Type_AES_t AES1;
    struct key_Type_WEP_t WEP;
    struct key_Type_WAPI_t WAPI;
...
typedef union anon_union.conflict1173_for_intf anon_union.conflict1173_for_intf, *Panon_union.conflict1173_for_intf;

union anon_union.conflict1173_for_intf { // DWARF DIE: 1173
    uint32 Interface;
};

struct BufferDesc { // DWARF DIE: 1252
    union anon_union.conflict1173_for_intf intf;
    uint16 DataLen;
    undefined field_0x6;
    undefined field_0x7;
    void * Buffer;
...
typedef union anon_union.conflict1173 anon_union.conflict1173, *Panon_union.conflict1173;
...
typedef union anon_union.conflict2f9f_for_specDat anon_union.conflict2f9f_for_specDat, *Panon_union.conflict2f9f_for_specDat;
...
struct AkmSuite_t { // DWARF DIE: 2176
    UINT8 akmOui[3];
    AkmTypePacked_e akmType;
};

struct MIC_Error_t { // DWARF DIE: 2355
...
struct SecurityParams_t { // DWARF DIE: 21a6
    struct SecurityMode_t wpaType;
    struct Cipher_t mcstCipher;
    struct Cipher_t ucstCipher;
};

struct customMIB_RSNStats_t { // DWARF DIE: 2392
    UINT8 TKIPICVErrors;
    UINT8 TKIPLocalMICFailures;
    UINT8 TKIPCounterMeasuresInvoked;
};

struct BandConfig_t { // DWARF DIE: 1a53
...
struct ChanBandInfo_t { // DWARF DIE: 1aaf
    struct BandConfig_t bandConfig;
    UINT8 chanNum;
};

struct apSpecificData_t { // DWARF DIE: 2f43
...
union anon_union.conflict2f9f_for_specDat { // DWARF DIE: 2f9f
    struct apSpecificData_t apData;
};

struct keyMgmtInfoSta_t { // DWARF DIE: 23cf
    UINT8 ANonce[32];
    UINT8 SNonce[32];
    UINT8 EAPOL_MIC_Key[16];
    UINT8 EAPOL_Encr_Key[16];
...
struct IEEEtypes_RSNCapability_t { // DWARF DIE: 17e3
    UINT8 PreAuth:1;
    UINT8 NoPairwise:1;
...
struct RSNConfig_t { // DWARF DIE: 2267
    UINT8 RSNEnabled:1;
    UINT8 pmkidValid:1;
    UINT8 rsnCapValid:1;
...
struct IEEEtypes_SsIdElement_t { // DWARF DIE: 16fb
    enum IEEEtypes_ElementId_e ElementId;
    IEEEtypes_Len_t Len;
    IEEEtypes_SsId_t SsId;
};

struct supplicantData { // DWARF DIE: 282a
    BOOLEAN inUse;
...
struct eapolHskData_t { // DWARF DIE: 1f53
    UINT8 ANonce[32];
    struct KeyData_t pwsKeyData;
};

union ckd { // DWARF DIE: 1f83
    struct eapolHskData_t hskData;
};

struct cipher_key_t { // DWARF DIE: 1f9d
    union ckd ckd;
};

struct cipher_key_buf { // DWARF DIE: 1fc4
    struct cipher_key_t cipher_key;
};

struct cm_ConnectionInfo { // DWARF DIE: 1195
    UINT8 conType;
    UINT8 staId;
    UINT8 instNbr;
    UINT8 gtkHwKeyId;
    ...
    union anon_union.conflict2f9f_for_specDat specDat;
    cipher_key_buf_t TxRxCipherKeyBuf;
};

union anon_union.conflict1173 { // DWARF DIE: 1173
    uint32 Interface;
    struct cm_ConnectionInfo * connPtr;
};
...
struct Hdr_8021x_t { // DWARF DIE: 1b29
    UINT8 protocol_ver;
...
struct key_info_t { // DWARF DIE: 1b66
    UINT16 KeyMIC:1;
    UINT16 Secure:1;
    UINT16 Error:1;
    UINT16 Request:1;
...
struct EAPOL_KeyMsg_t { // DWARF DIE: 1c2c
...
struct ether_hdr_t { // DWARF DIE: 1ae9
    IEEEtypes_MacAddr_t da;
    IEEEtypes_MacAddr_t sa;
    UINT16 type;
};

struct EAPOL_KeyMsg_Tx_t { // DWARF DIE: 1cee
...
struct rx_swdesc { // DWARF DIE: ebe
};

typedef struct cm_ConnectionInfo cm_ConnectionInfo_t;

typedef union anon_union.conflict2f9f anon_union.conflict2f9f, *Panon_union.conflict2f9f;

union anon_union.conflict2f9f { // DWARF DIE: 2f9f
    struct apSpecificData_t apData;
};

typedef struct phy_channel_info phy_channel_info, *Pphy_channel_info;

struct phy_channel_info { // DWARF DIE: a60
    uint32_t info1;
    uint32_t info2;
};
...
struct tx_cfm_tag { // DWARF DIE: ff9
    uint16_t pn[4];
    uint16_t sn;
    uint16_t timestamp;
    int8_t credits;
...
typedef union anon_union.conflictcc5 anon_union.conflictcc5, *Panon_union.conflictcc5;

union anon_union.conflictcc5 { // DWARF DIE: cc5
    uint32_t dataendptr;
...
typedef union anon_union.conflictc7b_for_field_3 anon_union.conflictc7b_for_field_3, *Panon_union.conflictc7b_for_field_3;

typedef union anon_union.conflictca0_for_field_4 anon_union.conflictca0_for_field_4, *Panon_union.conflictca0_for_field_4;

typedef union anon_union.conflictcc5_for_field_5 anon_union.conflictcc5_for_field_5, *Panon_union.conflictcc5_for_field_5;

union anon_union.conflictcc5_for_field_5 { // DWARF DIE: cc5
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};

union anon_union.conflictca0_for_field_4 { // DWARF DIE: ca0
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};

union anon_union.conflictc7b_for_field_3 { // DWARF DIE: c7b
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

struct tx_hd { // DWARF DIE: cea
    uint32_t upatterntx;
    uint32_t nextfrmexseq_ptr;
    uint32_t nextmpdudesc_ptr;
    union anon_union.conflictc7b_for_field_3 field_3;
    union anon_union.conflictca0_for_field_4 field_4;
    union anon_union.conflictcc5_for_field_5 field_5;
    uint32_t frmlen;
...
struct tx_hw_desc { // DWARF DIE: 1068
    struct tx_cfm_tag * cfm_ptr;
    struct tx_hd thd;
};

typedef union anon_union.conflictc7b anon_union.conflictc7b, *Panon_union.conflictc7b;

union anon_union.conflictc7b { // DWARF DIE: c7b
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

typedef union anon_union.conflictca0 anon_union.conflictca0, *Panon_union.conflictca0;

union anon_union.conflictca0 { // DWARF DIE: ca0
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};
...
struct rx_payloaddesc { // DWARF DIE: f9c
    struct rx_pbd pbd;
    uint32_t pd_status;
    uint32_t * buffer_rx;
    void * pbuf_holder[6];
...
struct rx_dmadesc { // DWARF DIE: f2c
    struct rx_hd hd;
...
struct IEEEtypes_WPAElement_t { // DWARF DIE: 174d
    enum IEEEtypes_ElementId_e ElementId;
    IEEEtypes_Len_t Len;
    UINT8 OuiType[4];
    UINT16 Ver;
...
struct IEEEtypes_RSNElement_t { // DWARF DIE: 18c7
    enum IEEEtypes_ElementId_e ElementId;
    IEEEtypes_Len_t Len;
    UINT16 Ver;
    UINT8 GrpKeyCipher[4];
