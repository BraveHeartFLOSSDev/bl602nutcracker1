...
typedef union anon_union.conflict2137 anon_union.conflict2137, *Panon_union.conflict2137;
...
union anon_union.conflict2137 { // DWARF DIE: 2137
...
typedef union anon_union.conflict2137_for_key anon_union.conflict2137_for_key, *Panon_union.conflict2137_for_key;

typedef char SINT8;

union anon_union.conflict2137_for_key { // DWARF DIE: 2137
    IEEEtypes_MacAddr_t Bssid;
    char Ssid[32];
};

struct pmkElement_t { // DWARF DIE: 2169
    union anon_union.conflict2137_for_key key;
    UINT8 PMK[32];
    UINT8 length;
    UINT8 psk_length;
...
struct co_list_hdr { // DWARF DIE: 1a3f
    struct co_list_hdr * next;
};

struct co_list { // DWARF DIE: 1a60
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct mm_timer_env_tag { // DWARF DIE: 1ad5
    struct co_list prog;
};

typedef struct mm_timer_tag mm_timer_tag, *Pmm_timer_tag;

struct mm_timer_tag { // DWARF DIE: 1a94
...
struct Cipher_t { // DWARF DIE: 1837
    UINT8 wep40:1;
    UINT8 wep104:1;
    UINT8 tkip:1;
    UINT8 ccmp:1;
...
struct KeyData_t { // DWARF DIE: 156a
    UINT8 Key[16];
    UINT8 RxMICKey[8];
    UINT8 TxMICKey[8];
    UINT32 TxIV32;
...
struct CommonMlmeData_t { // DWARF DIE: 2469
...
struct Operation_t { // DWARF DIE: 2553
    UINT16 keyExchange:1;
    UINT16 authenticate:1;
    UINT16 reserved:14;
};

struct apRsnConfig_t { // DWARF DIE: 259e
...
struct BssConfig_t { // DWARF DIE: 26a4
...
struct BssData_t { // DWARF DIE: 26f4
...
struct apInfo_t { // DWARF DIE: 2794
...
struct _txQingInfo_t { // DWARF DIE: 2529
...
struct SecurityMode_t { // DWARF DIE: 189d
    UINT16 noRsn:1;
    UINT16 wepStatic:1;
    UINT16 wepDynamic:1;
    UINT16 wpa:1;
...
struct apKeyMgmtInfoStaRom_t { // DWARF DIE: 237b
    UINT16 staRsnCap;
...
struct apKeyMgmtInfoSta_t { // DWARF DIE: 23d2
...
struct staData_t { // DWARF DIE: 2760
...
typedef union anon_union.conflict170f_for_keyEncypt anon_union.conflict170f_for_keyEncypt, *Panon_union.conflict170f_for_keyEncypt;
...
struct key_Type_AES_CMAC_t { // DWARF DIE: 16d2
    UINT8 ipn[6];
    UINT8 reserved[2];
    UINT8 key[16];
};

struct key_Type_TKIP_t { // DWARF DIE: 15ce
    UINT8 key[16];
    UINT8 txMicKey[8];
    UINT8 rxMicKey[8];
};

struct key_Type_WAPI_t { // DWARF DIE: 167b
    UINT8 keyIndex;
    UINT8 isDefKey;
    UINT8 key[16];
    UINT8 mickey[16];
...
struct key_Type_AES_t { // DWARF DIE: 1658
    UINT8 key[16];
};

struct key_Type_WEP_t { // DWARF DIE: 160b
    UINT8 keyIndex;
    UINT8 isDefaultTx;
    UINT8 key[13];
};

union anon_union.conflict170f_for_keyEncypt { // DWARF DIE: 170f
...
struct key_MgtMaterial_t { // DWARF DIE: 1755
    UINT16 keyType;
    UINT16 keyInfo;
    UINT16 keyLen;
    union anon_union.conflict170f_for_keyEncypt keyEncypt;
};

typedef union anon_union.conflict170f anon_union.conflict170f, *Panon_union.conflict170f;

union anon_union.conflict170f { // DWARF DIE: 170f
    struct key_Type_TKIP_t TKIP;
    struct key_Type_AES_t AES1;
    struct key_Type_WEP_t WEP;
    struct key_Type_WAPI_t WAPI;
...
typedef union anon_union.conflict1255_for_intf anon_union.conflict1255_for_intf, *Panon_union.conflict1255_for_intf;

union anon_union.conflict1255_for_intf { // DWARF DIE: 1255
    uint32 Interface;
};

struct BufferDesc { // DWARF DIE: 1334
    union anon_union.conflict1255_for_intf intf;
    uint16 DataLen;
    undefined field_0x6;
    undefined field_0x7;
    void * Buffer;
...
typedef union anon_union.conflict2834_for_specDat anon_union.conflict2834_for_specDat, *Panon_union.conflict2834_for_specDat;
...
struct AkmSuite_t { // DWARF DIE: 19c2
    UINT8 akmOui[3];
    AkmTypePacked_e akmType;
};

struct MIC_Error_t { // DWARF DIE: 1bea
...
struct SecurityParams_t { // DWARF DIE: 19f2
    struct SecurityMode_t wpaType;
    struct Cipher_t mcstCipher;
    struct Cipher_t ucstCipher;
};

struct customMIB_RSNStats_t { // DWARF DIE: 1c27
    UINT8 TKIPICVErrors;
    UINT8 TKIPLocalMICFailures;
    UINT8 TKIPCounterMeasuresInvoked;
};

struct BandConfig_t { // DWARF DIE: 117f
...
struct ChanBandInfo_t { // DWARF DIE: 11db
    struct BandConfig_t bandConfig;
    UINT8 chanNum;
};

struct apSpecificData_t { // DWARF DIE: 27d8
...
struct IEEEtypes_RSNCapability_t { // DWARF DIE: fcf
    UINT8 PreAuth:1;
    UINT8 NoPairwise:1;
...
struct RSNConfig_t { // DWARF DIE: 1afc
    UINT8 RSNEnabled:1;
    UINT8 pmkidValid:1;
    UINT8 rsnCapValid:1;
...
struct keyMgmtInfoSta_t { // DWARF DIE: 1c64
    UINT8 ANonce[32];
    UINT8 SNonce[32];
    UINT8 EAPOL_MIC_Key[16];
    UINT8 EAPOL_Encr_Key[16];
...
struct IEEEtypes_SsIdElement_t { // DWARF DIE: f7d
    enum IEEEtypes_ElementId_e ElementId;
    IEEEtypes_Len_t Len;
    IEEEtypes_SsId_t SsId;
};

struct supplicantData { // DWARF DIE: 20bf
    BOOLEAN inUse;
...
struct eapolHskData_t { // DWARF DIE: 179f
    UINT8 ANonce[32];
    struct KeyData_t pwsKeyData;
};

union ckd { // DWARF DIE: 17cf
    struct eapolHskData_t hskData;
};

struct cipher_key_t { // DWARF DIE: 17e9
    union ckd ckd;
};

struct cipher_key_buf { // DWARF DIE: 1810
    struct cipher_key_t cipher_key;
};

union anon_union.conflict2834_for_specDat { // DWARF DIE: 2834
    struct apSpecificData_t apData;
};

struct cm_ConnectionInfo { // DWARF DIE: 1277
    UINT8 conType;
    UINT8 staId;
    UINT8 instNbr;
    UINT8 gtkHwKeyId;
    ...
    union anon_union.conflict2834_for_specDat specDat;
...
struct Hdr_8021x_t { // DWARF DIE: 1375
    UINT8 protocol_ver;
...
struct key_info_t { // DWARF DIE: 13b2
    UINT16 KeyMIC:1;
    UINT16 Secure:1;
    UINT16 Error:1;
    UINT16 Request:1;
...
struct EAPOL_KeyMsg_t { // DWARF DIE: 1478
...
struct ether_hdr_t { // DWARF DIE: 1215
    IEEEtypes_MacAddr_t da;
    IEEEtypes_MacAddr_t sa;
    UINT16 type;
};

struct EAPOL_KeyMsg_Tx_t { // DWARF DIE: 153a
...
typedef enum anon_enum_8.conflict285f {
...
} anon_enum_8.conflict285f;

typedef union anon_union.conflict2834 anon_union.conflict2834, *Panon_union.conflict2834;

union anon_union.conflict2834 { // DWARF DIE: 2834
    struct apSpecificData_t apData;
};
...

{
  ...
  if (puVar2[1] == 0) {
    ...
    uVar3 = FUN_00010058();
    *(undefined4 *)(extraout_a0 + 8) = uVar3;
    puVar2[1] = extraout_a0;
    *puVar2 = *(undefined4 *)(extraout_a0 + 8);
    _L0();
  }
  return FW_SUCCESS;
}
...

{
  ...
  if (extraout_a0 != 0) {
    uVar2 = FUN_00010058();
    *(undefined4 *)(extraout_a0 + 8) = uVar2;
    unaff_s1[1] = extraout_a0;
    *unaff_s1 = *(undefined4 *)(extraout_a0 + 8);
    _L0();
  }
...

{
  ...
  *(undefined4 *)(unaff_s0 + 8) = uVar1;
  unaff_s1[1] = unaff_s0;
  *unaff_s1 = *(undefined4 *)(unaff_s0 + 8);
  _L0();
  return 0;
}
...
{
  cm_ConnectionInfo_t *pcVar1;
  int iVar2;
  
  pcVar1 = (cm_ConnectionInfo_t *)_L0();
  if (pcVar1 != (cm_ConnectionInfo_t *)0x0) {
    FUN_000100be();
    pcVar1->conType = conType;
    iVar2 = _L0();
    if (iVar2 == 1) {
      FUN_000100dc();
      pcVar1 = (cm_ConnectionInfo_t *)0x0;
    }
    else {
...

{
  ...
  if ((connPtr->conType == '\x02') && (_L0(), extraout_a0[1] != 0)) {
    FUN_00010178();
    FUN_00010182();
    extraout_a0[1] = 0;
    *extraout_a0 = 0;
...
{
  IEEEtypes_Len_t extraout_a0;
  uint uVar1;
  
  if (ssid != (char *)0x0) {
    FUN_000101f2();
    (connPtr->comData).SsIdLen = extraout_a0;
    FUN_00010208();
    uVar1 = (uint)(connPtr->comData).SsIdLen;
    if (uVar1 < 0x20) {
      (connPtr->comData).SsId[uVar1] = '\0';
    }
    return;
  }
  return;
...

{
  ...
  FUN_000101f2();
  *(undefined *)(unaff_s0 + 0x2c) = extraout_a0;
  FUN_00010208();
  if (*(byte *)(unaff_s0 + 0x2c) < 0x20) {
    *(undefined *)(unaff_s0 + (uint)*(byte *)(unaff_s0 + 0x2c) + 0xc) = 0;
  }
  return;
}
