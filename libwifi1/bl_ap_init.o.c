...
union anon_union.conflict2113 { // DWARF DIE: 2113
...
union anon_union.conflict2113_for_key { // DWARF DIE: 2113
    IEEEtypes_MacAddr_t Bssid;
    char Ssid[32];
};

struct pmkElement_t { // DWARF DIE: 2145
...
struct co_list_hdr { // DWARF DIE: 1a1b
    struct co_list_hdr * next;
};

struct co_list { // DWARF DIE: 1a3c
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct mm_timer_env_tag { // DWARF DIE: 1ab1
    struct co_list prog;
};

typedef struct mm_timer_tag mm_timer_tag, *Pmm_timer_tag;

struct mm_timer_tag { // DWARF DIE: 1a70
...
struct Cipher_t { // DWARF DIE: 1813
    UINT8 wep40:1;
    UINT8 wep104:1;
    UINT8 tkip:1;
    UINT8 ccmp:1;
...
struct KeyData_t { // DWARF DIE: 1546
    UINT8 Key[16];
    UINT8 RxMICKey[8];
    UINT8 TxMICKey[8];
    UINT32 TxIV32;
...
struct CommonMlmeData_t { // DWARF DIE: 2445
...
struct Operation_t { // DWARF DIE: 252f
    UINT16 keyExchange:1;
    UINT16 authenticate:1;
    UINT16 reserved:14;
};

struct apRsnConfig_t { // DWARF DIE: 257a
...
struct BssConfig_t { // DWARF DIE: 2680
...
struct BssData_t { // DWARF DIE: 26d0
...
struct apInfo_t { // DWARF DIE: 2770
...
struct _txQingInfo_t { // DWARF DIE: 2505
...
struct SecurityMode_t { // DWARF DIE: 1879
    UINT16 noRsn:1;
    UINT16 wepStatic:1;
    UINT16 wepDynamic:1;
    UINT16 wpa:1;
...
struct apKeyMgmtInfoStaRom_t { // DWARF DIE: 2357
    UINT16 staRsnCap;
...
struct apKeyMgmtInfoSta_t { // DWARF DIE: 23ae
...
struct staData_t { // DWARF DIE: 273c
...
struct key_Type_TKIP_t { // DWARF DIE: 15aa
    UINT8 key[16];
    UINT8 txMicKey[8];
    UINT8 rxMicKey[8];
};

struct key_Type_AES_t { // DWARF DIE: 1634
    UINT8 key[16];
};

struct key_Type_AES_CMAC_t { // DWARF DIE: 16ae
    UINT8 ipn[6];
    UINT8 reserved[2];
    UINT8 key[16];
};

struct key_Type_WAPI_t { // DWARF DIE: 1657
    UINT8 keyIndex;
    UINT8 isDefKey;
    UINT8 key[16];
    UINT8 mickey[16];
...
struct key_Type_WEP_t { // DWARF DIE: 15e7
    UINT8 keyIndex;
    UINT8 isDefaultTx;
    UINT8 key[13];
};

union anon_union.conflict16eb { // DWARF DIE: 16eb
...
union anon_union.conflict16eb_for_keyEncypt { // DWARF DIE: 16eb
    struct key_Type_TKIP_t TKIP;
    struct key_Type_AES_t AES1;
    struct key_Type_WEP_t WEP;
    struct key_Type_WAPI_t WAPI;
...
struct key_MgtMaterial_t { // DWARF DIE: 1731
    UINT16 keyType;
    UINT16 keyInfo;
    UINT16 keyLen;
...
union anon_union.conflictb01_for_intf { // DWARF DIE: b01
    uint32 Interface;
};

struct BufferDesc { // DWARF DIE: be0
...
struct AkmSuite_t { // DWARF DIE: 199e
    UINT8 akmOui[3];
    AkmTypePacked_e akmType;
};

struct MIC_Error_t { // DWARF DIE: 1bc6
...
struct SecurityParams_t { // DWARF DIE: 19ce
    struct SecurityMode_t wpaType;
    struct Cipher_t mcstCipher;
    struct Cipher_t ucstCipher;
};

union anon_union.conflictb01 { // DWARF DIE: b01
    uint32 Interface;
    struct cm_ConnectionInfo * connPtr;
};

struct customMIB_RSNStats_t { // DWARF DIE: 1c03
    UINT8 TKIPICVErrors;
    UINT8 TKIPLocalMICFailures;
    UINT8 TKIPCounterMeasuresInvoked;
};

struct BandConfig_t { // DWARF DIE: 127b
...
struct ChanBandInfo_t { // DWARF DIE: 12d7
    struct BandConfig_t bandConfig;
    UINT8 chanNum;
};

struct apSpecificData_t { // DWARF DIE: 27b4
...
struct keyMgmtInfoSta_t { // DWARF DIE: 1c40
    UINT8 ANonce[32];
    UINT8 SNonce[32];
    UINT8 EAPOL_MIC_Key[16];
    UINT8 EAPOL_Encr_Key[16];
...
struct IEEEtypes_SsIdElement_t { // DWARF DIE: 1079
    enum IEEEtypes_ElementId_e ElementId;
    IEEEtypes_Len_t Len;
    IEEEtypes_SsId_t SsId;
};

struct IEEEtypes_RSNCapability_t { // DWARF DIE: 10cb
    UINT8 PreAuth:1;
    UINT8 NoPairwise:1;
...
struct RSNConfig_t { // DWARF DIE: 1ad8
    UINT8 RSNEnabled:1;
    UINT8 pmkidValid:1;
    UINT8 rsnCapValid:1;
...
struct supplicantData { // DWARF DIE: 209b
    BOOLEAN inUse;
...
struct eapolHskData_t { // DWARF DIE: 177b
    UINT8 ANonce[32];
    struct KeyData_t pwsKeyData;
};

union ckd { // DWARF DIE: 17ab
    struct eapolHskData_t hskData;
};

struct cipher_key_t { // DWARF DIE: 17c5
    union ckd ckd;
};

struct cipher_key_buf { // DWARF DIE: 17ec
    struct cipher_key_t cipher_key;
};

union anon_union.conflict2810_for_specDat { // DWARF DIE: 2810
    struct apSpecificData_t apData;
};

struct cm_ConnectionInfo { // DWARF DIE: b23
    UINT8 conType;
    UINT8 staId;
    UINT8 instNbr;
    UINT8 gtkHwKeyId;
...
struct Hdr_8021x_t { // DWARF DIE: 1351
    UINT8 protocol_ver;
...
struct key_info_t { // DWARF DIE: 138e
    UINT16 KeyMIC:1;
    UINT16 Secure:1;
    UINT16 Error:1;
    UINT16 Request:1;
...
struct EAPOL_KeyMsg_t { // DWARF DIE: 1454
...
struct ether_hdr_t { // DWARF DIE: 1311
    IEEEtypes_MacAddr_t da;
    IEEEtypes_MacAddr_t sa;
    UINT16 type;
};

struct EAPOL_KeyMsg_Tx_t { // DWARF DIE: 1516
...
union anon_union.conflict2810 { // DWARF DIE: 2810
    struct apSpecificData_t apData;
};
...

{
  ...
  _LVL2();
  *(undefined *)(extraout_a0_01 + 0xa4) = extraout_a0;
  FUN_00010032();
  FUN_0001003c();
  *(undefined *)(extraout_a0_01 + 0x58) = extraout_a0_00;
  do {
...

{
  ...
  _LVL2();
  *(undefined *)(unaff_s0 + 0xa4) = extraout_a0;
  FUN_00010032();
  FUN_0001003c();
  *(undefined *)(unaff_s0 + 0x58) = extraout_a0_00;
  do {
                    // WARNING: Do nothing block with infinite loop
  } while( true );
}
...

{
  ...
  FUN_00010032();
  FUN_0001003c();
  *(undefined *)(unaff_s0 + 0x58) = extraout_a0;
  do {
                    // WARNING: Do nothing block with infinite loop
  } while( true );
}
...

{
  ...
  _L0();
  FUN_00010082();
  if ((extraout_a0_00 != 0) && (extraout_a0_01 != 0)) {
    _LVL17();
    *(undefined *)(extraout_a0_00 + 0xa4) = extraout_a0;
    FUN_000100ae();
    *(undefined4 *)(extraout_a0_00 + 0xb8) = 1;
    FUN_000100c6();
    *(undefined2 *)(extraout_a0_00 + 0x10) = 1;
    *(ushort *)(extraout_a0_00 + 8) = *(ushort *)(extraout_a0_00 + 8) & 0xf7 | 0x108;
    *(ushort *)(extraout_a0_00 + 0xc) = *(ushort *)(extraout_a0_00 + 0xc) & 0xf7 | 0x108;
    *(undefined *)(extraout_a0_00 + 0x79) = 3;
    *(undefined *)(extraout_a0_00 + 0x7a) = 3;
    *(undefined4 *)(extraout_a0_00 + 0x7c) = 100;
    *(undefined4 *)(extraout_a0_00 + 0x80) = 100;
    *(undefined4 *)(extraout_a0_00 + 0x14) = 0x15180;
  }
  return;
}
...

{
  ...
  FUN_00010082();
  if ((unaff_s0 != 0) && (extraout_a0_00 != 0)) {
    _LVL17();
    *(undefined *)(unaff_s0 + 0xa4) = extraout_a0;
    FUN_000100ae();
