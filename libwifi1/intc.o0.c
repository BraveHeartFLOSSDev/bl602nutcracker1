...
struct ipc_a2e_msg { // DWARF DIE: 1741
    uint32_t dummy_word;
    uint32_t msg[127];
};
...
struct hostdesc { // DWARF DIE: 14ac
    uint32_t pbuf_addr;
...
struct txdesc_host { // DWARF DIE: 16b5
    uint32_t ready;
...
struct ipc_shared_env_tag { // DWARF DIE: 177f
...
struct la_mem_format { // DWARF DIE: e99
    uint32_t word[4];
};
...
typedef union anon_union.conflictf09_for_field_3 anon_union.conflictf09_for_field_3, *Panon_union.conflictf09_for_field_3;

typedef union anon_union.conflictf2e_for_field_4 anon_union.conflictf2e_for_field_4, *Panon_union.conflictf2e_for_field_4;

typedef union anon_union.conflictf53_for_field_5 anon_union.conflictf53_for_field_5, *Panon_union.conflictf53_for_field_5;

struct co_list_hdr { // DWARF DIE: e28
    struct co_list_hdr * next;
};

struct tx_cfm_tag { // DWARF DIE: 1311
    uint16_t pn[4];
    uint16_t sn;
    uint16_t timestamp;
    int8_t credits;
...
struct lmacdesc { // DWARF DIE: 15ef
    struct tx_agg_desc * agg_desc;
    struct txl_buffer_tag * buffer;
    struct tx_hw_desc * hw_desc;
};

struct umacdesc { // DWARF DIE: 1588
...
union anon_union.conflictf2e_for_field_4 { // DWARF DIE: f2e
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};

union anon_union.conflictf53_for_field_5 { // DWARF DIE: f53
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};

union anon_union.conflictf09_for_field_3 { // DWARF DIE: f09
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

struct tx_hd { // DWARF DIE: f78
    uint32_t upatterntx;
    uint32_t nextfrmexseq_ptr;
    uint32_t nextmpdudesc_ptr;
    union anon_union.conflictf09_for_field_3 field_3;
    union anon_union.conflictf2e_for_field_4 field_4;
    union anon_union.conflictf53_for_field_5 field_5;
    uint32_t frmlen;
...
struct tx_hw_desc { // DWARF DIE: 1390
    struct tx_cfm_tag * cfm_ptr;
    struct tx_hd thd;
};

struct txdesc { // DWARF DIE: 163b
...
struct txl_buffer_tag { // DWARF DIE: 162a
};

struct txl_buffer_control { // DWARF DIE: 15e4
};

struct tx_agg_desc { // DWARF DIE: 13c1
    uint8_t reserved;
};

typedef struct dma_desc dma_desc, *Pdma_desc;

struct dma_desc { // DWARF DIE: d97
    uint32_t src;
    uint32_t dest;
    uint16_t length;
    uint16_t ctrl;
...
struct dma_env_tag { // DWARF DIE: deb
...
struct co_list { // DWARF DIE: e49
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct mm_timer_env_tag { // DWARF DIE: 1bcb
    struct co_list prog;
};
...
struct rx_swdesc { // DWARF DIE: 114c
    struct co_list_hdr list_hdr;
...
struct phy_channel_info { // DWARF DIE: e71
    uint32_t info1;
    uint32_t info2;
};

struct rx_pbd { // DWARF DIE: 11e1
    uint32_t upattern;
    uint32_t next;
    uint32_t datastartptr;
    uint32_t dataendptr;
...
struct rx_payloaddesc { // DWARF DIE: 12b4
    struct rx_pbd pbd;
    uint32_t pd_status;
    uint32_t * buffer_rx;
    void * pbuf_holder[6];
...
struct rx_dmadesc { // DWARF DIE: 1244
    struct rx_hd hd;
...
struct debug_env_tag { // DWARF DIE: ed1
...
struct rxl_cntrl_env_tag { // DWARF DIE: 242e
...
struct mblock_free { // DWARF DIE: 1839
};

struct ke_env_tag { // DWARF DIE: 17ea
...
struct txl_list { // DWARF DIE: 2322
...
struct txl_cntrl_env_tag { // DWARF DIE: 236a
...
struct hal_dma_env_tag { // DWARF DIE: 24aa
...
struct bam_env_tag { // DWARF DIE: 1ee9
    uint32_t pkt_cnt;
...
struct sta_mgmt_ba_info { // DWARF DIE: 20a3
...
typedef union anon_union.conflictada_for_u anon_union.conflictada_for_u, *Panon_union.conflictada_for_u;

typedef struct anon_struct.conflicta7a anon_struct.conflicta7a, *Panon_struct.conflicta7a;

typedef struct anon_struct.conflictab1 anon_struct.conflictab1, *Panon_struct.conflictab1;

struct rc_sta_stats { // DWARF DIE: 2098
};

struct sta_pol_tbl_cntl { // DWARF DIE: 2049
...
struct mac_rateset { // DWARF DIE: a32
    uint8_t length;
    uint8_t array[12];
};

struct mac_htcapability { // DWARF DIE: b85
...
struct mac_vhtcapability { // DWARF DIE: bf8
...
struct mac_sta_info { // DWARF DIE: c4d
...
struct anon_struct.conflictab1 { // DWARF DIE: ab1
    uint32_t key[4];
};

struct anon_struct.conflicta7a { // DWARF DIE: a7a
    uint32_t tx_key[2];
    uint32_t rx_key[2];
};

union anon_union.conflictada_for_u { // DWARF DIE: ada
    struct anon_struct.conflicta7a mic;
    struct anon_struct.conflictab1 mfp;
};

struct key_info_tag { // DWARF DIE: aff
    uint64_t rx_pn[9];
    uint64_t tx_pn;
    union anon_union.conflictada_for_u u;
    uint8_t cipher;
    uint8_t key_idx;
    uint8_t hw_key_idx;
    _Bool valid;
...
struct sta_info_env_tag { // DWARF DIE: 22db
...
struct txl_cfm_env_tag { // DWARF DIE: 23f7
    struct co_list cfmlist[5];
};

typedef union anon_union.conflictada anon_union.conflictada, *Panon_union.conflictada;

union anon_union.conflictada { // DWARF DIE: ada
    struct anon_struct.conflicta7a mic;
    struct anon_struct.conflictab1 mfp;
};
...
struct ke_msg_handler { // DWARF DIE: 1aef
    ke_msg_id_t id;
    undefined field_0x2;
    undefined field_0x3;
...
struct ke_state_handler { // DWARF DIE: 1b1b
...
typedef union anon_union.conflictf2e anon_union.conflictf2e, *Panon_union.conflictf2e;

union anon_union.conflictf2e { // DWARF DIE: f2e
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};

typedef union anon_union.conflictf09 anon_union.conflictf09, *Panon_union.conflictf09;

union anon_union.conflictf09 { // DWARF DIE: f09
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

typedef union anon_union.conflictf53 anon_union.conflictf53, *Panon_union.conflictf53;

union anon_union.conflictf53 { // DWARF DIE: f53
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};
...
struct ipc_emb_env_tag { // DWARF DIE: 1907
...
struct rx_cntrl_ipcdesc { // DWARF DIE: 1d1b
    uint32_t host_id;
};
...
struct rx_cntrl_pm_mon { // DWARF DIE: 1d63
    struct mac_addr addr;
    uint8_t pm_state;
    _Bool mon;
};

struct rx_cntrl_dupli { // DWARF DIE: 1d38
    struct mac_addr last_src_addr;
    uint16_t last_seq_cntl;
};

struct rx_cntrl_rx_status { // DWARF DIE: 1c20
    uint16_t frame_cntl;
    uint16_t seq_cntl;
    uint16_t sn;
    uint8_t fn;
...
struct rxu_cntrl_env_tag { // DWARF DIE: 1d9c
