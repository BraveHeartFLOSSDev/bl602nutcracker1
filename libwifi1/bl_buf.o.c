...
struct la_mem_format { // DWARF DIE: a54
    uint32_t word[4];
};
...
struct debug_env_tag { // DWARF DIE: a8c
...
wpabuf * wpabuf_alloc(size_t len)

{
  wpabuf *pwVar1;
  
  pwVar1 = (wpabuf *)FUN_00010012();
...
void * wpabuf_put(wpabuf *buf,size_t len)

{
  ...
  pwVar3 = (wpabuf *)buf->ext_data;
  if (pwVar3 == (wpabuf *)0x0) {
    pwVar3 = buf + 1;
  }
  sVar1 = buf->used;
  uVar2 = sVar1 + len;
  ...
  return (void *)((int)&pwVar3->size + sVar1);
}
