...
struct phyif_utils_recvtable_t { // DWARF DIE: 96e
    uint32_t recvtable1;
    uint32_t recvtable2;
    uint32_t recvtable3;
    uint32_t recvtable4;
...
struct phyif_utils_recvtable_priv_t { // DWARF DIE: 9d2
...

{
  ...
  if (((*(ushort *)((int)&vec->recvtable2 + 2) & 7) < 2) && ((vec->recvtable1 >> 0xc & 0xf) < 4)) {
    ...
    iVar1 = (int8_t)((int)CONCAT11(*(undefined *)&vec->recvtable6,
                                   *(undefined *)((int)&vec->recvtable5 + 3)) / 0x7a);
  }
  *ppm = iVar1;
  return 0;
}
