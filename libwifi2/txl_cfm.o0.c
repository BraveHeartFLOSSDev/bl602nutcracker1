...
struct ipc_a2e_msg { // DWARF DIE: 395a
    uint32_t dummy_word;
    uint32_t msg[127];
};
...
struct hostdesc { // DWARF DIE: 16c5
    uint32_t pbuf_addr;
...
struct txdesc_host { // DWARF DIE: 19c7
    uint32_t ready;
...
struct ipc_shared_env_tag { // DWARF DIE: 3998
...
struct co_list_hdr { // DWARF DIE: e51
    struct co_list_hdr * next;
};

union anon_union.conflict10f2_for_field_4 { // DWARF DIE: 10f2
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};

struct tx_cfm_tag { // DWARF DIE: 152a
    uint16_t pn[4];
    uint16_t sn;
    uint16_t timestamp;
    int8_t credits;
...
struct umacdesc { // DWARF DIE: 17a1
...
union anon_union.conflict10cd_for_field_3 { // DWARF DIE: 10cd
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

union anon_union.conflict1117_for_field_5 { // DWARF DIE: 1117
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};

struct tx_hd { // DWARF DIE: 113c
    uint32_t upatterntx;
...
struct tx_hw_desc { // DWARF DIE: 15a9
    struct tx_cfm_tag * cfm_ptr;
    struct tx_hd thd;
};

struct lmacdesc { // DWARF DIE: 1831
    struct tx_agg_desc * agg_desc;
    struct txl_buffer_tag * buffer;
    struct tx_hw_desc * hw_desc;
};

struct txdesc { // DWARF DIE: 194d
...
struct tx_compressed_policy_tbl { // DWARF DIE: 10a2
    uint32_t upatterntx;
    uint32_t sec_user_control;
};

struct tx_pbd { // DWARF DIE: 1205
    uint32_t upatterntx;
    uint32_t next;
    uint32_t datastartptr;
    uint32_t dataendptr;
...
struct tx_policy_tbl { // DWARF DIE: 1031
    uint32_t upatterntx;
...
union anon_union.conflict28fa_for_field_0 { // DWARF DIE: 28fa
    struct tx_policy_tbl policy_tbl;
    struct tx_compressed_policy_tbl comp_pol_tbl;
};

struct txl_buffer_control { // DWARF DIE: 17fd
    union anon_union.conflict28fa_for_field_0 field_0;
    uint32_t mac_control_info;
    uint32_t phy_control_info;
};

struct txl_buffer_tag { // DWARF DIE: 186c
    uint32_t length;
    uint32_t lenheader;
    uint32_t lenpad;
    uint32_t flags;
...
struct tx_agg_desc { // DWARF DIE: 15da
    uint8_t reserved;
};
...
struct mm_timer_tag { // DWARF DIE: 1e16
    struct co_list_hdr list_hdr;
    void (* cb)(void *);
    void * env;
    uint32_t time;
...
struct ps_env_tag { // DWARF DIE: 38be
    _Bool ps_on;
    undefined field_0x1;
    ke_task_id_t taskid;
...
struct co_list { // DWARF DIE: e77
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct txl_list { // DWARF DIE: 2709
...
struct txl_cntrl_env_tag { // DWARF DIE: 2751
...
struct txl_frame_env_tag { // DWARF DIE: 2a6c
...
struct txl_frame_cfm_tag { // DWARF DIE: 29f0
    void (* cfm_func)(void *, uint32_t);
    void * env;
};

struct txl_frame_desc_tag { // DWARF DIE: 2a18
    struct txdesc txdesc;
...
struct bam_env_tag { // DWARF DIE: 2175
    uint32_t pkt_cnt;
...
struct chan_tbtt_tag { // DWARF DIE: 2bf6
    struct co_list_hdr list_hdr;
    uint32_t time;
    uint8_t vif_index;
    uint8_t priority;
...
struct mm_chan_ctxt_add_req { // DWARF DIE: 1d4a
    uint8_t band;
    uint8_t type;
...
struct chan_ctxt_tag { // DWARF DIE: 2c45
    struct co_list_hdr list_hdr;
...
struct chan_env_tag { // DWARF DIE: 2cd5
...
struct mblock_free { // DWARF DIE: 1ab3
};

typedef struct scan_start_req scan_start_req, *Pscan_start_req;

struct scan_start_req { // DWARF DIE: 2f51
};
...
struct me_chan_config_req { // DWARF DIE: 30b5
...
struct me_env_tag { // DWARF DIE: 327d
...
struct mobility_domain { // DWARF DIE: 3311
    uint16_t mdid;
...
struct Cipher_t { // DWARF DIE: 3161
    UINT8 wep40:1;
    UINT8 wep104:1;
    UINT8 tkip:1;
    UINT8 ccmp:1;
...
struct SecurityMode_t { // DWARF DIE: 31c7
    UINT16 noRsn:1;
    UINT16 wepStatic:1;
    UINT16 wepDynamic:1;
    UINT16 wpa:1;
...
struct mac_bss_info { // DWARF DIE: 3339
    struct mac_htcapability ht_cap;
...
struct txl_cfm_env_tag { // DWARF DIE: 27de
...
struct ke_msg_handler { // DWARF DIE: 1cba
    ke_msg_id_t id;
    undefined field_0x2;
    undefined field_0x3;
...
struct ke_state_handler { // DWARF DIE: 1ce6
...
struct ipc_emb_env_tag { // DWARF DIE: 39f2
...
struct rx_cntrl_ipcdesc { // DWARF DIE: 1fa7
    uint32_t host_id;
};
...
struct rx_cntrl_pm_mon { // DWARF DIE: 1fef
    struct mac_addr addr;
    uint8_t pm_state;
    _Bool mon;
};

struct rx_cntrl_dupli { // DWARF DIE: 1fc4
    struct mac_addr last_src_addr;
    uint16_t last_seq_cntl;
};

struct rx_cntrl_rx_status { // DWARF DIE: 1eac
    uint16_t frame_cntl;
    uint16_t seq_cntl;
    uint16_t sn;
    uint8_t fn;
...
struct rxu_cntrl_env_tag { // DWARF DIE: 2028
...
struct la_mem_format { // DWARF DIE: fc1
    uint32_t word[4];
};

typedef struct dma_env_tag dma_env_tag, *Pdma_env_tag;

struct dma_env_tag { // DWARF DIE: f5c
    struct dma_desc * last_dma[4];
};

typedef struct mm_timer_env_tag mm_timer_env_tag, *Pmm_timer_env_tag;

struct mm_timer_env_tag { // DWARF DIE: 1e57
    struct co_list prog;
};
...
struct rx_swdesc { // DWARF DIE: 1365
    struct co_list_hdr list_hdr;
...
struct phy_channel_info { // DWARF DIE: f99
    uint32_t info1;
    uint32_t info2;
};

struct rx_pbd { // DWARF DIE: 13fa
    uint32_t upattern;
    uint32_t next;
    uint32_t datastartptr;
    uint32_t dataendptr;
...
struct rx_payloaddesc { // DWARF DIE: 14cd
    struct rx_pbd pbd;
    uint32_t pd_status;
    uint32_t * buffer_rx;
    void * pbuf_holder[6];
...
struct rx_dmadesc { // DWARF DIE: 145d
    struct rx_hd hd;
...
struct debug_env_tag { // DWARF DIE: ff9
...
struct ke_env_tag { // DWARF DIE: 1a64
...
struct anon_struct.conflict35e6 { // DWARF DIE: 35e6
    uint32_t dummy;
...
struct anon_struct.conflict3519 { // DWARF DIE: 3519
...
union anon_union.conflict36e1 { // DWARF DIE: 36e1
...
union anon_union.conflict36e1_for_u { // DWARF DIE: 36e1
    struct anon_struct.conflict3519 sta;
    struct anon_struct.conflict35e6 ap;
};

struct vif_info_tag { // DWARF DIE: 3704
    struct co_list_hdr list_hdr;
    uint32_t prevent_sleep;
...
struct vif_mgmt_env_tag { // DWARF DIE: 383f
...
struct hal_dma_env_tag { // DWARF DIE: 2e4f
...
struct hal_dma_desc_tag { // DWARF DIE: 2e08
...
struct sta_mgmt_ba_info { // DWARF DIE: 248a
...
struct rc_rate_stats { // DWARF DIE: 2fd3
    uint16_t attempts;
    uint16_t success;
...
union anon_union.conflict3070_for_rate_map { // DWARF DIE: 3070
    uint8_t ht[4];
};

struct rc_sta_stats { // DWARF DIE: 2324
...
struct sta_pol_tbl_cntl { // DWARF DIE: 22d5
...
struct scan_probe_req_ie_tag { // DWARF DIE: 2eb0
    struct dma_desc dma_desc;
    struct tx_pbd pbd;
    uint32_t buf[50];
};
...
struct txl_buffer_list_tag { // DWARF DIE: 2849
...
struct txl_buffer_hw_desc_tag { // DWARF DIE: 2821
    struct dma_desc dma_desc;
    struct tx_pbd pbd;
};
...
struct txl_buffer_idx_tag { // DWARF DIE: 2871
    uint32_t used_area;
    uint32_t free;
    uint32_t free_size;
    uint32_t last;
...
union anon_union.conflict28fa { // DWARF DIE: 28fa
    struct tx_policy_tbl policy_tbl;
    struct tx_compressed_policy_tbl comp_pol_tbl;
};
...
struct txl_buffer_env_tag { // DWARF DIE: 291c
...
union anon_union.conflict10f2 { // DWARF DIE: 10f2
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};
...
union anon_union.conflict10cd { // DWARF DIE: 10cd
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};
...
union anon_union.conflict1117 { // DWARF DIE: 1117
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};
...

{
  ...
  if ((uRam00000000 & *(uint *)(access_category * 4)) == 0) {
    FUN_00010112(_DAT_00000050);
  }
  _L0();
  while (iVar1 = *(int *)(access_category * 8), iVar1 != 0) {
    FUN_0001013a();
    FUN_00010148(iVar1);
    FUN_00010152(iVar1);
    _DAT_00000050 += -1;
...

{
  ...
  while (FUN_00010216(), extraout_a0 != 0) {
    iVar1 = **(int **)(extraout_a0 + 0x6c);
    if (*(int *)(extraout_a0 + 100) == 0) {
      if (-1 < *(int *)(iVar1 + 0x10)) {
        *(uint32_t *)(iVar1 + 0x10) = status;
      }
    }
    else {
      *(uint32_t *)(iVar1 + 0x10) = status | 0x3c000000;
    }
    if (*(int *)(extraout_a0 + 8) == 0) {
      _L0(extraout_a0);
    }
    else {
      _L0(extraout_a0);
      _DAT_00000050 += -1;
      if (*(int *)(extraout_a0 + 0x68) != 0) {
        *(undefined4 *)(extraout_a0 + 0x68) = 0;
      }
    }
  }
  _L0();
