...
struct ipc_a2e_msg { // DWARF DIE: 3e95
    uint32_t dummy_word;
    uint32_t msg[127];
};
...
struct hostdesc { // DWARF DIE: 2a17
    uint32_t pbuf_addr;
...
struct txdesc_host { // DWARF DIE: 2ce5
    uint32_t ready;
...
struct ipc_shared_env_tag { // DWARF DIE: 3ed3
...
struct co_list_hdr { // DWARF DIE: e4f
    struct co_list_hdr * next;
};

union anon_union.conflict1177_for_field_4 { // DWARF DIE: 1177
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};

struct tx_cfm_tag { // DWARF DIE: 15af
    uint16_t pn[4];
    uint16_t sn;
    uint16_t timestamp;
    int8_t credits;
...
union anon_union.conflict1152_for_field_3 { // DWARF DIE: 1152
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};

struct umacdesc { // DWARF DIE: 2af3
...
union anon_union.conflict119c_for_field_5 { // DWARF DIE: 119c
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};

struct tx_hd { // DWARF DIE: 11c1
    uint32_t upatterntx;
...
struct tx_hw_desc { // DWARF DIE: 162e
    struct tx_cfm_tag * cfm_ptr;
    struct tx_hd thd;
};

struct lmacdesc { // DWARF DIE: 2b4f
    struct tx_agg_desc * agg_desc;
    struct txl_buffer_tag * buffer;
    struct tx_hw_desc * hw_desc;
};

struct txdesc { // DWARF DIE: 2c6b
...
struct tx_compressed_policy_tbl { // DWARF DIE: 1127
    uint32_t upatterntx;
    uint32_t sec_user_control;
};

struct tx_policy_tbl { // DWARF DIE: 10b6
    uint32_t upatterntx;
...
union anon_union.conflict2fa6_for_field_0 { // DWARF DIE: 2fa6
    struct tx_policy_tbl policy_tbl;
    struct tx_compressed_policy_tbl comp_pol_tbl;
};

struct txl_buffer_control { // DWARF DIE: 235b
    union anon_union.conflict2fa6_for_field_0 field_0;
    uint32_t mac_control_info;
    uint32_t phy_control_info;
};

struct tx_pbd { // DWARF DIE: 128a
    uint32_t upatterntx;
    uint32_t next;
    uint32_t datastartptr;
    uint32_t dataendptr;
...
struct txl_buffer_tag { // DWARF DIE: 2b8a
    uint32_t length;
    uint32_t lenheader;
    uint32_t lenpad;
    uint32_t flags;
...
struct tx_agg_desc { // DWARF DIE: 165f
    uint8_t reserved;
};
...
struct mm_timer_tag { // DWARF DIE: 1e10
    struct co_list_hdr list_hdr;
    void (* cb)(void *);
    void * env;
    uint32_t time;
...
struct ps_env_tag { // DWARF DIE: 3d23
    _Bool ps_on;
    undefined field_0x1;
    ke_task_id_t taskid;
...
struct co_list { // DWARF DIE: e75
    struct co_list_hdr * first;
    struct co_list_hdr * last;
};

struct txl_list { // DWARF DIE: 2e04
...
struct txl_cntrl_env_tag { // DWARF DIE: 2e4c
...
struct rd_env_tag { // DWARF DIE: 3cfc
...
struct txl_frame_cfm_tag { // DWARF DIE: 309c
    void (* cfm_func)(void *, uint32_t);
    void * env;
};

struct txl_frame_desc_tag { // DWARF DIE: 30c4
    struct txdesc txdesc;
...
struct txl_frame_env_tag { // DWARF DIE: 3118
...
struct bam_env_tag { // DWARF DIE: 216f
    uint32_t pkt_cnt;
...
struct chan_tbtt_tag { // DWARF DIE: 2848
    struct co_list_hdr list_hdr;
    uint32_t time;
    uint8_t vif_index;
    uint8_t priority;
...
struct mm_chan_ctxt_add_req { // DWARF DIE: 1b41
    uint8_t band;
    uint8_t type;
...
struct chan_ctxt_tag { // DWARF DIE: 2897
    struct co_list_hdr list_hdr;
...
struct chan_env_tag { // DWARF DIE: 2927
...
struct mblock_free { // DWARF DIE: 2ded
};

typedef struct scan_start_req scan_start_req, *Pscan_start_req;

struct scan_start_req { // DWARF DIE: 3343
};
...
struct me_chan_config_req { // DWARF DIE: 34a7
...
struct me_env_tag { // DWARF DIE: 366f
...
struct mobility_domain { // DWARF DIE: 3703
    uint16_t mdid;
...
struct Cipher_t { // DWARF DIE: 3553
    UINT8 wep40:1;
    UINT8 wep104:1;
    UINT8 tkip:1;
    UINT8 ccmp:1;
...
struct SecurityMode_t { // DWARF DIE: 35b9
    UINT16 noRsn:1;
    UINT16 wepStatic:1;
    UINT16 wepDynamic:1;
    UINT16 wpa:1;
...
struct mac_bss_info { // DWARF DIE: 372b
    struct mac_htcapability ht_cap;
...
struct ke_msg_handler { // DWARF DIE: 1846
    ke_msg_id_t id;
    undefined field_0x2;
    undefined field_0x3;
...
struct ke_state_handler { // DWARF DIE: 1872
...
struct mm_env_tag { // DWARF DIE: 2774
...
struct ipc_emb_env_tag { // DWARF DIE: 3f2d
...
struct rx_cntrl_pm_mon { // DWARF DIE: 1fe9
    struct mac_addr addr;
    uint8_t pm_state;
    _Bool mon;
};

struct rx_cntrl_dupli { // DWARF DIE: 1fbe
    struct mac_addr last_src_addr;
    uint16_t last_seq_cntl;
};

struct rx_cntrl_ipcdesc { // DWARF DIE: 1fa1
    uint32_t host_id;
};

struct rx_cntrl_rx_status { // DWARF DIE: 1ea6
    uint16_t frame_cntl;
    uint16_t seq_cntl;
    uint16_t sn;
    uint8_t fn;
...
struct rxu_cntrl_env_tag { // DWARF DIE: 2022
...
struct hal_dma_desc_tag { // DWARF DIE: 1d4b
...
struct mm_bcn_env_tag { // DWARF DIE: 3e1a
...
struct mm_bcn_change_req { // DWARF DIE: 1ba4
    uint32_t bcn_ptr;
    uint16_t bcn_len;
    uint16_t tim_oft;
    uint8_t tim_len;
...
struct la_mem_format { // DWARF DIE: 1046
    uint32_t word[4];
};

typedef struct dma_env_tag dma_env_tag, *Pdma_env_tag;

struct dma_env_tag { // DWARF DIE: fc0
    struct dma_desc * last_dma[4];
};

typedef struct mm_timer_env_tag mm_timer_env_tag, *Pmm_timer_env_tag;

struct mm_timer_env_tag { // DWARF DIE: 1e51
    struct co_list prog;
};
...
struct rx_swdesc { // DWARF DIE: 13ea
    struct co_list_hdr list_hdr;
...
struct phy_channel_info { // DWARF DIE: 101e
    uint32_t info1;
    uint32_t info2;
};

struct rx_pbd { // DWARF DIE: 147f
    uint32_t upattern;
    uint32_t next;
    uint32_t datastartptr;
    uint32_t dataendptr;
...
struct rx_payloaddesc { // DWARF DIE: 1552
    struct rx_pbd pbd;
    uint32_t pd_status;
    uint32_t * buffer_rx;
    void * pbuf_holder[6];
...
struct rx_dmadesc { // DWARF DIE: 14e2
    struct rx_hd hd;
...
struct mm_traffic_req_ind { // DWARF DIE: 1c58
    uint8_t sta_idx;
    uint8_t pkt_cnt;
    _Bool uapsd;
};
...
struct mm_csa_counter_ind { // DWARF DIE: 1c91
    uint8_t vif_index;
    uint8_t csa_count;
};
...
struct mm_tim_update_req { // DWARF DIE: 1c1a
    uint16_t aid;
    uint8_t tx_avail;
    uint8_t inst_nbr;
};
...
struct debug_env_tag { // DWARF DIE: 107e
...
struct rxl_cntrl_env_tag { // DWARF DIE: 3dbf
...
struct ke_env_tag { // DWARF DIE: 2d9e
...
struct anon_struct.conflict3a24 { // DWARF DIE: 3a24
    uint32_t dummy;
...
struct anon_struct.conflict3957 { // DWARF DIE: 3957
...
union anon_union.conflict3b1f { // DWARF DIE: 3b1f
...
union anon_union.conflict3b1f_for_u { // DWARF DIE: 3b1f
    struct anon_struct.conflict3957 sta;
    struct anon_struct.conflict3a24 ap;
};

struct vif_info_tag { // DWARF DIE: 3b42
    struct co_list_hdr list_hdr;
    uint32_t prevent_sleep;
...
struct vif_mgmt_env_tag { // DWARF DIE: 3c7d
...
struct hal_dma_env_tag { // DWARF DIE: 1d92
...
struct sta_mgmt_ba_info { // DWARF DIE: 24f5
...
struct rc_rate_stats { // DWARF DIE: 33c5
    uint16_t attempts;
    uint16_t success;
...
union anon_union.conflict3462_for_rate_map { // DWARF DIE: 3462
    uint8_t ht[4];
};

struct rc_sta_stats { // DWARF DIE: 238f
...
struct sta_pol_tbl_cntl { // DWARF DIE: 230c
...
struct sta_info_env_tag { // DWARF DIE: 272d
...
union anon_union.conflict3462 { // DWARF DIE: 3462
    uint8_t ht[4];
};

typedef struct scan_env_tag scan_env_tag, *Pscan_env_tag;

struct scan_env_tag { // DWARF DIE: 32e7
...
struct scan_probe_req_ie_tag { // DWARF DIE: 32a2
    struct dma_desc dma_desc;
    struct tx_pbd pbd;
    uint32_t buf[50];
};
...
struct txl_buffer_list_tag { // DWARF DIE: 2ef5
...
struct txl_buffer_hw_desc_tag { // DWARF DIE: 2ecd
    struct dma_desc dma_desc;
    struct tx_pbd pbd;
};
...
struct txl_buffer_idx_tag { // DWARF DIE: 2f1d
    uint32_t used_area;
    uint32_t free;
    uint32_t free_size;
    uint32_t last;
...
struct txl_buffer_env_tag { // DWARF DIE: 2fc8
...
union anon_union.conflict2fa6 { // DWARF DIE: 2fa6
    struct tx_policy_tbl policy_tbl;
    struct tx_compressed_policy_tbl comp_pol_tbl;
};
...
union anon_union.conflict119c { // DWARF DIE: 119c
    uint32_t dataendptr;
    uint32_t sec_user3_ptr;
};
...
union anon_union.conflict1177 { // DWARF DIE: 1177
    uint32_t datastartptr;
    uint32_t sec_user2_ptr;
};
...
union anon_union.conflict1152 { // DWARF DIE: 1152
    uint32_t first_pbd_ptr;
    uint32_t sec_user1_ptr;
};
...

{
  ...
  bVar1 = vif_entry->index;
  FUN_0001034a();
  *(uint *)((uint)bVar1 * 0x48) = (uint)vif_entry->index * 0x14;
  ...
  *puVar6 = 5;
  puVar6[1] = 4;
  uVar2 = *(undefined *)&(vif_entry->u).field_0x2e7;
  puVar6[4] = 0;
  puVar6[2] = uVar2;
  puVar6[3] = 1;
  puVar6[5] = 0xff;
  puVar5[5] = 0xcafefade;
  bVar1 = *(byte *)&(vif_entry->u).field_0x2e9;
  puVar5[6] = puVar4;
  puVar5[8] = (uint)bVar1 + uVar3 * 0xfc;
  _L0();
  *puVar4 = 0xcafefade;
  puVar4[1] = 0;
  puVar4[4] = 0;
...

{
  ...
  if (_DAT_00000004 != 0) {
    FUN_000104c2(_DAT_00000018,_DAT_0000001c,_DAT_00000054);
  }
  _DAT_00000009 = (uint3)((uint)_DAT_00000008 >> 8);
  if (DAT_00000009 == '\0') {
    _DAT_00000008 = (int **)((uint)_DAT_00000009 << 8);
    for (; ppiVar3 != (int **)0x0; ppiVar3 = (int **)*ppiVar3) {
      if (((*(char *)((int)ppiVar3 + 0x56) == '\x02') && (*(char *)((int)ppiVar3 + 0x342) != '\0'))
         && (*(char *)((int)ppiVar3 + 0x341) == *(char *)(ppiVar3 + 0xd0))) {
        piVar4 = ppiVar3[0x33];
        iVar8 = (uint)*(byte *)((int)ppiVar3 + 0x57) * 8;
        bVar1 = *(byte *)(iVar8 + 4);
        piVar4[7] = (uint)*(ushort *)(ppiVar3 + 0xce) + (uint)*(ushort *)((int)ppiVar3 + 0x33a) + 4;
        uVar6 = (uint)_DAT_00000054;
        _DAT_00000054 = (ushort)((uVar6 + 1) * 0x10000 >> 0x10);
        *(short *)(piVar4[5] + 0x16) = (short)((uVar6 + 1 & 0xffff) << 4);
        cVar7 = *(char *)((int)ppiVar3 + 0x343);
        *(char *)(iVar8 + 2) = cVar7;
        if (cVar7 == '\0') {
          if ((*(char *)((int)ppiVar3 + 0x346) != '\0') ||
             (bVar5 = bVar1 & 0xfe,
             *(int *)((_DAT_00000018 & uRam00000000) * 0x330 + _DAT_0000001c) != 0)) {
            bVar5 = bVar1 | 1;
          }
          *(undefined *)((int)ppiVar3 + 0x343) = *(undefined *)(iVar8 + 3);
        }
        else {
          bVar5 = bVar1 & 0xfe;
          if ((*(byte *)((int)ppiVar3 + 0x346) & 2) != 0) {
            bVar5 = bVar1 | 1;
          }
        }
        *(byte *)(iVar8 + 4) = bVar5 | 1;
        *(char *)((int)ppiVar3 + 0x343) = *(char *)((int)ppiVar3 + 0x343) + -1;
        if (*(char *)((int)ppiVar3 + 0x347) != '\0') {
          cVar7 = *(char *)((int)ppiVar3 + 0x347) + -1;
          *(char *)((int)ppiVar3 + 0x347) = cVar7;
          if (*(byte *)(ppiVar3 + 0xd2) != 0) {
            *(char *)(piVar4[5] + (uint)*(byte *)(ppiVar3 + 0xd2)) = cVar7;
            if (*(byte *)((int)ppiVar3 + 0x349) != 0) {
              *(undefined *)(piVar4[5] + (uint)*(byte *)((int)ppiVar3 + 0x349)) =
                   *(undefined *)((int)ppiVar3 + 0x347);
            }
          }
          cVar7 = *(char *)((int)ppiVar3 + 0x347);
          if (cVar7 != '\0') {
            uVar2 = *(undefined *)((int)ppiVar3 + 0x57);
            FUN_000106bc(0x4e,0xd,0,2);
            *extraout_a0_02 = uVar2;
            extraout_a0_02[1] = cVar7;
            _L0();
            if (*(char *)((int)ppiVar3 + 0x347) != '\0') goto _L0;
          }
          *(undefined *)((int)ppiVar3 + 0x347) = 1;
        }
_L0:
        FUN_00010600(ppiVar3,ppiVar3 + 0x18);
        FUN_0001060a(ppiVar3);
        if (extraout_a0 != 0) {
          cVar7 = *(char *)((int)ppiVar3 + 0x57);
          FUN_00010624(0x4a,0xd,0,3);
          *extraout_a0_00 = cVar7 + '\n';
          extraout_a0_00[1] = '\0';
          extraout_a0_00[2] = '\0';
          _L0();
          *(undefined *)((int)ppiVar3 + 0x8f) = *(undefined *)((int)ppiVar3 + 0x57);
          *(undefined *)(ppiVar3 + 0x24) = 0xff;
          _L0((int)(ppiVar3 + 0x18),4);
          if (extraout_a0_01 != 0) {
            _DAT_00000004 += 1;
          }
          iVar8 = (*(byte *)((int)ppiVar3 + 0x57) + 10) * 0x1b0;
          if ((*(byte *)(iVar8 + 0x32) & 2) != 0) {
            *(undefined4 *)(iVar8 + 0x34) = 9;
            FUN_0001068c((int *)ppiVar3,iVar8,0);
    ...
    _DAT_00000008 = (int **)CONCAT31(_DAT_00000009,1);
  }
  return;
}
...

{
  ...
  FUN_000104c2(uRam00000018,iRam0000001c,uRam00000054);
    ...
    *(undefined *)(unaff_s2 + 8) = 0;
    for (; unaff_s0 != (int **)0x0; unaff_s0 = (int **)*unaff_s0) {
      if (((*(char *)((int)unaff_s0 + 0x56) == '\x02') && (*(char *)((int)unaff_s0 + 0x342) != '\0')
        ...
        piVar3 = unaff_s0[0x33];
        iVar7 = (uint)*(byte *)((int)unaff_s0 + 0x57) * 8;
        bVar1 = *(byte *)(iVar7 + 4);
        piVar3[7] = (uint)*(ushort *)(unaff_s0 + 0xce) + (uint)*(ushort *)((int)unaff_s0 + 0x33a) +
                    4;
        uVar5 = (uint)uRam00000054;
        uRam00000054 = (ushort)((uVar5 + 1) * 0x10000 >> 0x10);
        *(short *)(piVar3[5] + 0x16) = (short)((uVar5 + 1 & 0xffff) << 4);
        cVar6 = *(char *)((int)unaff_s0 + 0x343);
        *(char *)(iVar7 + 2) = cVar6;
        if (cVar6 == '\0') {
          ...
          *(undefined *)((int)unaff_s0 + 0x343) = *(undefined *)(iVar7 + 3);
        }
        else {
          bVar4 = bVar1 & 0xfe;
        ...
        *(byte *)(iVar7 + 4) = bVar4 | 1;
        *(char *)((int)unaff_s0 + 0x343) = *(char *)((int)unaff_s0 + 0x343) + -1;
        if (*(char *)((int)unaff_s0 + 0x347) != '\0') {
          cVar6 = *(char *)((int)unaff_s0 + 0x347) + -1;
          *(char *)((int)unaff_s0 + 0x347) = cVar6;
          if (*(byte *)(unaff_s0 + 0xd2) != 0) {
            *(char *)(piVar3[5] + (uint)*(byte *)(unaff_s0 + 0xd2)) = cVar6;
            if (*(byte *)((int)unaff_s0 + 0x349) != 0) {
              *(undefined *)(piVar3[5] + (uint)*(byte *)((int)unaff_s0 + 0x349)) =
                   *(undefined *)((int)unaff_s0 + 0x347);
            }
          }
          cVar6 = *(char *)((int)unaff_s0 + 0x347);
          if (cVar6 != '\0') {
            uVar2 = *(undefined *)((int)unaff_s0 + 0x57);
            FUN_000106bc(0x4e,0xd,0,2);
            *extraout_a0_02 = uVar2;
            extraout_a0_02[1] = cVar6;
            _L0();
...
_L0:
        ...
        FUN_0001060a(unaff_s0);
        if (extraout_a0 != 0) {
          cVar6 = *(char *)((int)unaff_s0 + 0x57);
          FUN_00010624(0x4a,0xd,0,3);
          *extraout_a0_00 = cVar6 + '\n';
          extraout_a0_00[1] = '\0';
          extraout_a0_00[2] = '\0';
          _L0();
          *(undefined *)((int)unaff_s0 + 0x8f) = *(undefined *)((int)unaff_s0 + 0x57);
          *(undefined *)(unaff_s0 + 0x24) = 0xff;
          _L0((int)(unaff_s0 + 0x18),4);
          if (extraout_a0_01 != 0) {
            *(int *)(unaff_s2 + 4) = *(int *)(unaff_s2 + 4) + 1;
          }
          iVar7 = (*(byte *)((int)unaff_s0 + 0x57) + 10) * 0x1b0;
          if ((*(byte *)(iVar7 + 0x32) & 2) != 0) {
            *(undefined4 *)(iVar7 + 0x34) = 9;
            FUN_0001068c((int *)unaff_s0,iVar7,0);
...
code_r0x00010600:
      ...
      *(undefined *)(unaff_s0 + 0x24) = 0xff;
      ...
      if ((*(byte *)(iVar3 + 0x32) & 2) != 0) {
        *(undefined4 *)(iVar3 + 0x34) = 9;
        FUN_0001068c((int *)unaff_s0,iVar3,0);
    ...
    do {
      ...
      if (unaff_s0 == (int **)0x0) {
        return;
            ...
            (*(char *)((int)unaff_s0 + 0x341) != *(char *)(unaff_s0 + 0xd0)));
    piVar4 = unaff_s0[0x33];
    iVar8 = (uint)*(byte *)((int)unaff_s0 + 0x57) * 8 + unaff_s5;
    bVar1 = *(byte *)(iVar8 + 4);
    piVar4[7] = (uint)*(ushort *)(unaff_s0 + 0xce) + (uint)*(ushort *)((int)unaff_s0 + 0x33a) + 4;
    iVar3 = piVar4[5];
    unaff_s1 = unaff_s0 + 0x18;
    uVar6 = *(ushort *)(unaff_s3 + 0x54) + 1;
    *(short *)(unaff_s3 + 0x54) = (short)(uVar6 * 0x10000 >> 0x10);
    *(short *)(iVar3 + 0x16) = (short)((uVar6 & 0xffff) << 4);
    cVar7 = *(char *)((int)unaff_s0 + 0x343);
    *(char *)(iVar8 + 2) = cVar7;
    if (cVar7 == '\0') {
      if ((*(char *)((int)unaff_s0 + 0x346) != '\0') ||
         (bVar5 = bVar1 & 0xfe,
      ...
      *(undefined *)((int)unaff_s0 + 0x343) = *(undefined *)(iVar8 + 3);
    }
    else {
      bVar5 = bVar1 & 0xfe;
      if ((*(byte *)((int)unaff_s0 + 0x346) & 2) != 0) {
  ...
  cVar7 = *(char *)((int)unaff_s0 + 0x347) + -1;
  *(char *)((int)unaff_s0 + 0x347) = cVar7;
  if (*(byte *)(unaff_s0 + 0xd2) != 0) {
    *(char *)(piVar4[5] + (uint)*(byte *)(unaff_s0 + 0xd2)) = cVar7;
    if (*(byte *)((int)unaff_s0 + 0x349) != 0) {
      *(undefined *)(piVar4[5] + (uint)*(byte *)((int)unaff_s0 + 0x349)) =
           *(undefined *)((int)unaff_s0 + 0x347);
    }
  }
  cVar7 = *(char *)((int)unaff_s0 + 0x347);
  if (cVar7 != '\0') goto _L0;
  goto _L0;
_L0:
  uVar2 = *(undefined *)((int)unaff_s0 + 0x57);
...

{
  ...
  do {
    FUN_00010624(param_1,param_2,param_3,param_4);
    *extraout_a0_00 = unaff_s7;
    extraout_a0_00[1] = '\0';
    extraout_a0_00[2] = '\0';
    _L0();
    *(undefined *)((int)unaff_s0 + 0x8f) = *(undefined *)((int)unaff_s0 + 0x57);
...
_L0:
      FUN_00010600(unaff_s0,(int **)param_1);
      FUN_0001060a(unaff_s0);
    } while (extraout_a0 == 0);
    cVar7 = *(char *)((int)unaff_s0 + 0x57);
    FUN_00010624(0x4a,0xd,0,3);
    ...
    *(undefined4 *)(unaff_s1 + 0x34) = 0;
    do {
      do {
        ...
        iVar8 = (uint)*(byte *)((int)unaff_s0 + 0x57) * 8 + unaff_s5;
        bVar1 = *(byte *)(iVar8 + 4);
        piVar4[7] = (uint)*(ushort *)(unaff_s0 + 0xce) + (uint)*(ushort *)((int)unaff_s0 + 0x33a) +
                    4;
        iVar3 = piVar4[5];
        uVar6 = *(ushort *)(unaff_s3 + 0x54) + 1;
        *(short *)(unaff_s3 + 0x54) = (short)(uVar6 * 0x10000 >> 0x10);
...

{
  ...
  do {
    FUN_000106bc(param_1,param_2,param_3,param_4);
    *extraout_a0_02 = unaff_s8;
    extraout_a0_02[1] = unaff_s7;
    _L0();
    if (*(char *)((int)unaff_s0 + 0x347) != '\0') goto _L0;
    do {
      *(undefined *)((int)unaff_s0 + 0x347) = 1;
          ...
          *(undefined *)(unaff_s0 + 0x24) = 0xff;
          ...
          if ((*(byte *)(iVar2 + 0x32) & 2) != 0) {
            *(undefined4 *)(iVar2 + 0x34) = 9;
            FUN_0001068c((int *)unaff_s0,iVar2,0);
        ...
        do {
          ...
          if (unaff_s0 == (int **)0x0) {
            return;
                ...
                (*(char *)((int)unaff_s0 + 0x341) != *(char *)(unaff_s0 + 0xd0)));
        piVar3 = unaff_s0[0x33];
        iVar7 = (uint)*(byte *)((int)unaff_s0 + 0x57) * 8 + unaff_s5;
        bVar1 = *(byte *)(iVar7 + 4);
        piVar3[7] = (uint)*(ushort *)(unaff_s0 + 0xce) + (uint)*(ushort *)((int)unaff_s0 + 0x33a) +
                    4;
        iVar2 = piVar3[5];
        unaff_s1 = unaff_s0 + 0x18;
        uVar5 = *(ushort *)(unaff_s3 + 0x54) + 1;
        *(short *)(unaff_s3 + 0x54) = (short)(uVar5 * 0x10000 >> 0x10);
        *(short *)(iVar2 + 0x16) = (short)((uVar5 & 0xffff) << 4);
        cVar6 = *(char *)((int)unaff_s0 + 0x343);
        *(char *)(iVar7 + 2) = cVar6;
        if (cVar6 == '\0') {
          if ((*(char *)((int)unaff_s0 + 0x346) != '\0') ||
...

{
  ...
  _L0(_DAT_00000024);
  iVar4 = iRam00000000;
  bVar1 = *(byte *)((int)env + 0x57);
  iVar5 = *(int *)((int)env + 0xcc);
  uVar2 = *(ushort *)(iRam00000000 + 6);
  iVar9 = (uint)*(ushort *)(iRam00000000 + 4) - (uint)*(byte *)(iRam00000000 + 8);
  iVar7 = *(int *)(iVar5 + 0x14);
  *(short *)((int)env + 0x338) = (short)iVar9;
  _L0(_DAT_00000024);
  iVar6 = *(int *)(iVar5 + 0x14) + (*(ushort *)(iVar4 + 6) - 1);
  *(int *)(iVar5 + 0x18) = iVar6;
  iVar6 = *(byte *)(iVar4 + 8) + 1 + iVar6;
  iVar8 = (uint)bVar1 * 0x14;
  *(int *)(iVar8 + 8) = iVar6;
  uVar3 = *(ushort *)(iVar4 + 6);
  *(undefined4 *)(iVar8 + 0x10) = 0;
  *(uint *)(iVar8 + 0xc) = ~(uint)uVar3 + iVar9 + iVar6;
  _DAT_00000024 = _DAT_44b000a0 & 0xff;
  *(undefined4 *)(iVar5 + 0x28) = 0;
  *(undefined4 *)(iVar5 + 0x24) = 0;
  *(undefined4 *)(iVar5 + 0x3c) = 0;
  *(uint *)(iVar5 + 0x10) = (uint)*(byte *)((int)env + 0x57) * 0x28;
  *(undefined *)((int)env + 0x342) = 1;
  *(undefined *)((uint)bVar1 * 8 + 3) = *(undefined *)(iVar7 + uVar2 + 3);
  iVar4 = iRam00000000;
  *(undefined *)((int)env + 0x347) = 0;
  *(undefined *)((int)env + 0x348) = *(undefined *)(iVar4 + 10);
  *(undefined *)((int)env + 0x349) = *(undefined *)(iVar4 + 0xb);
  if (*(byte *)(iVar4 + 10) != 0) {
    *(char *)((int)env + 0x347) =
         *(char *)(*(int *)(*(int *)((int)env + 0xcc) + 0x14) + (uint)*(byte *)(iVar4 + 10)) +
         '\x01';
  }
  DAT_00000009 = 0;
  if (DAT_00000008 != '\0') {
...

{
  ...
  _L0(uRam00000024);
  bVar1 = *(byte *)(unaff_s0 + 0x57);
  iVar6 = *unaff_s1;
  iVar4 = *(int *)(unaff_s0 + 0xcc);
  uVar2 = *(ushort *)(iVar6 + 6);
  iVar9 = (uint)*(ushort *)(iVar6 + 4) - (uint)*(byte *)(iVar6 + 8);
  iVar7 = *(int *)(iVar4 + 0x14);
  *(short *)(unaff_s0 + 0x338) = (short)iVar9;
  _L0(uRam00000024);
  iVar5 = *(int *)(iVar4 + 0x14) + (*(ushort *)(iVar6 + 6) - 1);
  *(int *)(iVar4 + 0x18) = iVar5;
  iVar5 = *(byte *)(iVar6 + 8) + 1 + iVar5;
  iVar8 = (uint)bVar1 * 0x14;
  *(int *)(iVar8 + 8) = iVar5;
  uVar3 = *(ushort *)(iVar6 + 6);
  *(undefined4 *)(iVar8 + 0x10) = 0;
  *(uint *)(iVar8 + 0xc) = ~(uint)uVar3 + iVar9 + iVar5;
  uRam00000024 = _DAT_44b000a0 & 0xff;
  *(undefined4 *)(iVar4 + 0x28) = 0;
  *(undefined4 *)(iVar4 + 0x24) = 0;
  *(undefined4 *)(iVar4 + 0x3c) = 0;
  *(uint *)(iVar4 + 0x10) = (uint)*(byte *)(unaff_s0 + 0x57) * 0x28;
  *(undefined *)(unaff_s0 + 0x342) = 1;
  *(undefined *)((uint)bVar1 * 8 + 3) = *(undefined *)(iVar7 + uVar2 + 3);
  iVar4 = *unaff_s8;
  *(undefined *)(unaff_s0 + 0x347) = 0;
  *(undefined *)(unaff_s0 + 0x348) = *(undefined *)(iVar4 + 10);
  *(undefined *)(unaff_s0 + 0x349) = *(undefined *)(iVar4 + 0xb);
  if (*(byte *)(iVar4 + 10) != 0) {
    *(char *)(unaff_s0 + 0x347) =
         *(char *)(*(int *)(*(int *)(unaff_s0 + 0xcc) + 0x14) + (uint)*(byte *)(iVar4 + 10)) +
         '\x01';
  }
...

{
  ...
  FUN_000108ec();
  iVar1 = *(int *)(unaff_s0 + 4) + -1;
  *(int *)(unaff_s0 + 4) = iVar1;
  if (iVar1 == 0) {
    if (*(char *)(unaff_s0 + 10) != '\0') {
      FUN_00010904();
    }
...

{
  ...
  while (FUN_00010940(param_1), *(int *)(unaff_s0 + 0x1c) != 0) {
    FUN_00010936(unaff_s2 + 0x1c);
    param_1 = extraout_a0 + 0xc;
  }
  if (*(char *)(unaff_s1 + 0x347) != '\x01') {
    return;
  }
